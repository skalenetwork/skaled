    ...
    for ( int stampDelta = 0; stampDelta < 45; stampDelta += 2 ) {
        for ( int pUncles = 0; pUncles < 2; pUncles++ ) {
            for ( int blockNumber = 100000; blockNumber < 5000000; blockNumber += 100000 ) {
                ...
                auto pStamp = static_cast< int64_t >(
                    test::RandomCode::get().randomUniInt( 1, 34020247236 ) );
                ...
                replaceMap["[CDIFF]"] =
                    toCompactHexPrefixed( _sealEngine.calculateDifficulty( current, parent ) );
...
void testDifficulty( fs::path const& _testFileFullName, Ethash& _sealEngine ) {
    ...
    for ( auto& i : v.get_obj() ) {
        ...
        BOOST_REQUIRE_MESSAGE(
            o.count( "parentTimestamp" ) > 0, testname + " missing parentTimestamp field" );
        BOOST_REQUIRE_MESSAGE(
            o.count( "parentDifficulty" ) > 0, testname + " missing parentDifficulty field" );
        BOOST_REQUIRE_MESSAGE(
            o.count( "currentBlockNumber" ) > 0, testname + " missing currentBlockNumber field" );
        BOOST_REQUIRE_MESSAGE(
            o.count( "parentUncles" ) > 0, testname + " missing parentUncles field" );
        BOOST_REQUIRE_MESSAGE(
            o.count( "currentTimestamp" ) > 0, testname + " missing currentTimestamp field" );
        BOOST_REQUIRE_MESSAGE(
            o.count( "currentBlockNumber" ) > 0, testname + " missing currentBlockNumber field" );
        BOOST_REQUIRE_MESSAGE(
            o.count( "currentDifficulty" ) > 0, testname + " missing currentDifficulty field" );
...
BOOST_AUTO_TEST_CASE( difficultyTestsFrontier ) {
    fs::path const testFileFullName =
        test::getTestPath() / fs::path( "BasicTests/difficultyFrontier.json" );
...
BOOST_AUTO_TEST_CASE( difficultyTestsHomestead ) {
    fs::path const testFileFullName =
        test::getTestPath() / fs::path( "BasicTests/difficultyHomestead.json" );
...
BOOST_AUTO_TEST_CASE( difficultyTestsCustomMainNetwork ) {
    fs::path const testFileFullName =
        test::getTestPath() / fs::path( "BasicTests/difficultyCustomMainNetwork.json" );
            ...
            for ( size_t pdN = 0; pdN < parentDifficultyVector.size(); pdN++ )
                for ( int pUncles = 0; pUncles < 3; pUncles++ )
                    for ( size_t tsN = 0; tsN < timestampDeltaVector.size(); tsN++ ) {
                        ...
                        auto cStamp = static_cast< int64_t >(
                            test::RandomCode::get().randomUniInt( timestampDeltaVector.back() ) );
                        ...
                        replaceMap["[CDIFF]"] = toCompactHexPrefixed(
                            sealEngine.calculateDifficulty( current, parent ) );

                        test::RandomCodeOptions defaultOptions;
                        test::RandomCode::get().parseTestWithTypes(
                            tmptest, replaceMap, defaultOptions );
                        finalTest << tmptest;
                    }

...
BOOST_AUTO_TEST_CASE( TransactionChainIDMax64Bit, 
    *boost::unit_test::precondition( dev::test::run_not_express ) ) {
    ...
    tx1.checkChainId( std::numeric_limits< uint64_t >::max(), true );
    ...
    tx2.checkChainId( std::numeric_limits< uint64_t >::max(), true );
}
...
BOOST_AUTO_TEST_CASE( TransactionReplayProtected ) {
    ...
    Transaction tx( txRlp, CheckTransaction::None );
    tx.checkChainId( 1, true );
    BOOST_REQUIRE_THROW( tx.checkChainId( 123, true ), InvalidSignature );

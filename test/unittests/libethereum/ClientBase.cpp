...
BOOST_AUTO_TEST_CASE( blocks ) {
    enumerateClients( []( Json::Value const& _json, dev::eth::ClientBase& _client ) -> void {
        ...
        for ( Json::Value const& block : _json["blocks"] ) {
            ...
            h256 hashFromNumber =
                _client.hashFromNumber( jsToInt( blockHeader["number"].asString() ) );
            ...
            auto compareBlockInfos = []( Json::Value const& _b, BlockHeader _blockInfo ) -> void {
                ...
                h256 expectedBlockInfoReceiptsRoot =
                    h256( fromHex( _b["receiptTrie"].asString() ) );
                u256 expectedBlockInfoTimestamp = u256( _b["timestamp"].asString() );
                h256 expectedBlockInfoTransactionsRoot =
                    h256( fromHex( _b["transactionsTrie"].asString() ) );
            ...
            BlockHeader blockInfo(
                ( static_cast< FixedClient& >( _client ) ).bc().headerData( blockHash ),
                HeaderData );
            ...
            auto compareTransactions = []( Json::Value const& _t,
                ...
                ETH_CHECK_EQUAL_COLLECTIONS( expectedTransactionData.begin(),
                    expectedTransactionData.end(), _transaction.data().begin(),
                ...
                //				ETH_CHECK_EQUAL(expectedTransactionSignatureV,
                //_transaction.signature().v); // 27 === 0x0, 28 === 0x1, not sure why
            };

...
public:
    void importAccount( h256 const& _addressHash, u256 const& _nonce, u256 const& _balance,
        ...
        if ( importedAccounts.count( _addressHash ) )
            ...
            importedAccounts.emplace( std::make_pair(
                _addressHash, ImportedAccount{_nonce, _balance, _storage, _codeHash} ) );
    }
    ...
    bool isAccountImported( h256 const& _addressHash ) const override {
        return importedAccounts.count( _addressHash ) != 0;
    }
    h256 stateRoot() const override { return h256{}; }
    std::string lookupCode( h256 const& _hash ) const override {
        auto it = std::find_if( importedCodes.begin(), importedCodes.end(),
            [&_hash]( bytes const& _code ) { return sha3( _code ) == _hash; } );
...
public:
    ...
    std::string readChunk( h256 const& _chunkHash ) const override {
        ...
        return it == chunks.end() ? std::string{} :
...
bytes createManifest( int _version, h256s const& _stateChunks, h256s const& _blockChunks,
    h256 const& _stateRoot, unsigned _blockNumber, h256 const& _blockHash ) {
    RLPStream s( 6 );
    s << _version;
...
bytes createAccount( u256 const& _nonce, u256 const& _balance, _byte_ _codeFlag, bytes const& _code,
    std::map< h256, bytes > _storage ) {
    RLPStream s( 5 );
...
bytes createAbridgedBlock( Address const& _author, h256 const& _stateRoot, h2048 const& _logBloom,
    u256 const& _difficulty, u256 const& _gasLimit, u256 const& _gasUsed, u256 const& _timestamp,
    bytes const& _extraData, h256 const& _mixHash, h64 const& _nonce, bytes const& _transactions,
    bytes const& _uncles ) {
    RLPStream s( 12 );
    s << _author << _stateRoot << _logBloom << _difficulty << _gasLimit << _gasUsed << _timestamp
      << _extraData;
...
bytes createSingleBlockChunk( unsigned _number, h256 const& _hash, u256 const& _totalDifficulty,
    bytes const& _abridgedBlock, bytes const& _receipts ) {
    RLPStream s( 4 );
...
BOOST_AUTO_TEST_CASE( SnapshotImporterSuite_importSplittedAccount,
    *boost::unit_test::precondition( dev::test::run_not_express ) ) {
    ...
    snapshotStorage.manifest =
        createManifest( 2, {stateChunk1, stateChunk2}, {}, h256{}, 0, h256{} );
...
BOOST_AUTO_TEST_CASE( SnapshotImporterSuite_importAccountsWithEqualCode,
    *boost::unit_test::precondition( dev::test::run_not_express ) ) {
    ...
    snapshotStorage.chunks[stateChunk] =
        createStateChunk( {{addressHash1, account1}, {addressHash2, account2}} );
...
BOOST_AUTO_TEST_CASE( SnapshotImporterSuite_importEmptyBlock,
    *boost::unit_test::precondition( dev::test::run_not_express ) ) {
    ...
    Nonce nonce( 012 );
    bytes block = createAbridgedBlock( author, stateRoot, logBloom, difficulty, gasLimit, gasUsed,
        timestamp, extraData, mixHash, nonce, RLPEmptyList, RLPEmptyList );

    unsigned parentNumber = 345;
    h256 parentHash = sha3( "678" );
    u256 parentTotalDifficulty = 910;
    bytes chunkBytes = createSingleBlockChunk(
        parentNumber, parentHash, parentTotalDifficulty, block, RLPEmptyList );
    ...
    BOOST_CHECK_EQUAL_COLLECTIONS(
        header.extraData().begin(), header.extraData().end(), extraData.begin(), extraData.end() );
...
BOOST_AUTO_TEST_CASE( SnapshotImporterSuite_importBlockWithTransactions,
    *boost::unit_test::precondition( dev::test::run_not_express ) ) {
    ...
    bytes block = createAbridgedBlock( Address( "111" ), sha3( "222" ), h2048( 333 ), 444, 555, 666,
        777, {8, 8, 8}, sha3( "999" ), Nonce( 012 ), transactions, uncles );
    ...
    BOOST_REQUIRE_EQUAL( blockChainImporter.importedBlocks.size(), 1 );
    ImportedBlock const& importedBlock = blockChainImporter.importedBlocks.front();
    BOOST_CHECK_EQUAL_COLLECTIONS( importedBlock.transactions.begin(),
        importedBlock.transactions.end(), transactions.begin(), transactions.end() );
    BOOST_CHECK_EQUAL_COLLECTIONS(
        importedBlock.uncles.begin(), importedBlock.uncles.end(), uncles.begin(), uncles.end() );
    BOOST_CHECK_EQUAL_COLLECTIONS( importedBlock.receipts.begin(), importedBlock.receipts.end(),

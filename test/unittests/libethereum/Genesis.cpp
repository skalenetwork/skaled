...
BOOST_AUTO_TEST_CASE( genesis_tests ) {
    ...
    BOOST_REQUIRE_MESSAGE( s.length() > 0,
        "Contents of 'genesishashestest.json' is empty. Have you cloned the 'tests' repo branch "
        "develop?" );
    js::read_string( s, v );

    js::mObject o = v.get_obj();

    ChainParams p( genesisInfo( eth::Network::MainNetwork ) );
    BOOST_CHECK_EQUAL(
        toHex( p.genesisBlock() ), toHex( fromHex( o["genesis_rlp_hex"].get_str() ) ) );
    BOOST_CHECK_EQUAL(
        BlockHeader( p.genesisBlock() ).hash(), h256( o["genesis_hash"].get_str() ) );
}

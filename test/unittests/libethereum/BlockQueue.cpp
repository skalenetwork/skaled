...
BOOST_AUTO_TEST_CASE( BlockQueueImport ) {
    ...
    BOOST_REQUIRE_MESSAGE( res == ImportResult::Success,
        "Simple block import to BlockQueue should have return Success" );

    res = blockQueue.import( &block1.bytes() );
    BOOST_REQUIRE_MESSAGE( res == ImportResult::AlreadyKnown,
        "Simple block import to BlockQueue should have return AlreadyKnown" );

    blockQueue.clear();
    blockchain.addBlock( block1 );
    res = blockQueue.import( &block1.bytes() );
    BOOST_REQUIRE_MESSAGE( res == ImportResult::AlreadyInChain,
        "Simple block import to BlockQueue should have return AlreadyInChain" );
    ...
    BOOST_REQUIRE_MESSAGE( res == ImportResult::Malformed,
        "Simple block import to BlockQueue should have return Malformed" );
    ...
    BOOST_REQUIRE_MESSAGE( res == ImportResult::UnknownParent,
        "Simple block import to BlockQueue should have return UnknownParent" );
}

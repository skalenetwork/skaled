        ...
        try {
            {  // block
                ...
                clog( VerbosityInfo, "TestClientFixture::getTransactionStatus()" )
                    << ( cc::debug( "Will compute status of transaction: " ) + cc::j( jo ) +
                           cc::debug( " ..." ) );
            }  // block
            ...
            {  // block
                ...
                nlohmann::json jo = nlohmann::json::parse( s );
                clog( VerbosityInfo, "TestClientFixture::getTransactionStatus()" )
                    << ( cc::debug( "Got transaction receipt: " ) + cc::j( jo ) +
                           cc::debug( " ..." ) );
            }  // block
...
BOOST_AUTO_TEST_CASE(
    ...
    BOOST_REQUIRE( fs::exists(
        fs::path( fixture.getTmpDataDir() ) / "snapshots" / "3" / "snapshot_hash.txt" ) );

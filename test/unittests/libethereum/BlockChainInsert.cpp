...
public:
    TestClient( Secret const& _authority )
        : m_path( boost::filesystem::temp_directory_path().string() + "/" +
          ...
          m_state( Invalid256, m_path, h256() ),
    ...
    void sealAndImport( Block& _block ) {
        ...
        m_bc.import( _block.blockData(), m_state );
        //		cdebug << "done.";
    }

    void import( Block const& _block ) { m_bc.import( _block.blockData(), m_state ); }
...
void syncStateTrie(
    ...
    while ( !todo.empty() ) {
        // h256 h = todo.back();
        todo.pop_back();
        throw std::logic_error( "Lookup is not available from this context." );
        //        bytes d = asBytes(_stateSource.lookup(h));
        //        cnote << h << ": " << RLP(&d);
        //        auto s = subs(&d);
        //        cnote << "   More: " << s;
        //        todo += s;
...
BOOST_AUTO_TEST_CASE( bcBasicInsert ) {
    ...
    Block block = tcFull.bc().genesisBlock( tcFull.state() );

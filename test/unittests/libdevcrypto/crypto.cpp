...
BOOST_AUTO_TEST_CASE( keypairs ) {
    KeyPair p(
        Secret( fromHex( "3ecb44df2159c26e0f995712d4f39b6f6e499b40749b1cf1246c37f9516cb6a4" ) ) );
    BOOST_REQUIRE(
        p.pub() ==
        Public( fromHex( "97466f2b32bc3bb76d4741ae51cd1d8578b48d3f1e68da206d47321aec267ce78549b514e"
                         "4453d74ef11b0cd5e4e4c364effddac8b51bcfc8de80682f952896f" ) ) );
    BOOST_REQUIRE(
...
BOOST_AUTO_TEST_CASE( SignAndRecover, 
    *boost::unit_test::precondition( dev::test::run_not_express ) ) {
    ...
    auto expectedSig =
        "b826808a8c41e00b7c5d71f211f005a84a7b97949d5e765831e1da4e34c9b8295d2a622eee50f25af78241c1cb"
        "7cfff11bcf2a13fe65dee1e3b86fd79a4e3ed000";
    BOOST_CHECK_EQUAL( sig.hex(), expectedSig );

    auto pub = recover( sig, msg );
    auto expectedPub =
        "e40930c838d6cca526795596e368d16083f0672f4ab61788277abfa23c3740e1cc84453b0b24f49086feba0bd9"
        "78bb4446bae8dff1e79fcc1e9cf482ec2d07c3";
...
BOOST_AUTO_TEST_CASE( ecies_standard ) {
    ...
    BOOST_REQUIRE(
        bytesConstRef( &b ).cropped( 0, original.size() ).toBytes() == asBytes( original ) );
}
...
BOOST_AUTO_TEST_CASE( cryptopp_aes128_cbc, 
    *boost::unit_test::precondition( dev::test::run_not_express ) ) {
    ...
    // Generate random IV
    _byte_ iv[CryptoPP::AES::BLOCKSIZE];
    ...
    string cipher;
    CryptoPP::StreamTransformationFilter* aesStream = new CryptoPP::StreamTransformationFilter(
        cbcEncryption, new CryptoPP::StringSink( cipher ) );
    CryptoPP::StringSource source( string192, true, aesStream );
    BOOST_REQUIRE( cipher.size() == 32 );

    _byte_* pOut = reinterpret_cast< _byte_* >( &string192[0] );
    _byte_ const* pIn = reinterpret_cast< _byte_ const* >( cipher.data() );
...
BOOST_AUTO_TEST_CASE( PerfSHA256_32,
    *utf::label( "perf" ) * boost::unit_test::precondition( dev::test::run_not_express ) ) {
...
BOOST_AUTO_TEST_CASE( PerfSHA256_4000,
    *utf::label( "perf" ) * boost::unit_test::precondition( dev::test::run_not_express ) ) {
    if ( !test::Options::get().all ) {

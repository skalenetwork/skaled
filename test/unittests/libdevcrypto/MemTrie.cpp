...
public:
    ...
    TrieBranchNode( _byte_ _i1, MemTrieNode* _n1, std::string const& _value = std::string() )
        : m_value( _value ) {
        memset( m_nodes.data(), 0, sizeof( MemTrieNode* ) * 16 );
        m_nodes[_i1] = _n1;
    }

    TrieBranchNode( _byte_ _i1, MemTrieNode* _n1, _byte_ _i2, MemTrieNode* _n2 ) {
        memset( m_nodes.data(), 0, sizeof( MemTrieNode* ) * 16 );
        m_nodes[_i1] = _n1;
        m_nodes[_i2] = _n2;
    }
...
private:
    /// @returns (_byte_)-1 when no active branches, 16 when multiple active and the index of the
    /// active branch otherwise.
    _byte_ activeBranch() const;
...
public:
    TrieLeafNode( bytesConstRef _key, std::string const& _value )
        : TrieExtNode( _key ), m_value( _value ) {}
...
MemTrieNode* MemTrieNode::newBranch(
    ...
    MemTrieNode* ret;
    if ( _k1.size() == prefix )
        ret = new TrieBranchNode(
            _k2[prefix], new TrieLeafNode( _k2.cropped( prefix + 1 ), _v2 ), _v1 );
    else if ( _k2.size() == prefix )
        ret = new TrieBranchNode(
            _k1[prefix], new TrieLeafNode( _k1.cropped( prefix + 1 ), _v1 ), _v2 );
...
MemTrieNode* TrieBranchNode::rejig() {
    mark();
    _byte_ n = activeBranch();

    if ( n == ( _byte_ ) -1 && m_value.size() ) {
        // switch to leaf
...
_byte_ TrieBranchNode::activeBranch() const {
    _byte_ n = ( _byte_ ) -1;
    for ( int i = 0; i < 16; ++i )
...
MemTrieNode* TrieInfixNode::insert( bytesConstRef _key, std::string const& _value ) {
    ...
    if ( contains( _key ) ) {
        ...
        if ( prefix ) {
            ...
            return new TrieInfixNode(
                _key.cropped( 0, prefix ), insert( _key.cropped( prefix ), _value ) );
        } else {
            // split here.
            auto f = m_ext[0];
            trimFront( m_ext, 1 );
...
void MemTrie::insert( std::string const& _key, std::string const& _value ) {
    ...
    auto h = asNibbles( _key );
    m_root =
        m_root ? m_root->insert( &h, _value ) : new TrieLeafNode( bytesConstRef( &h ), _value );
}

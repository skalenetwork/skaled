...
enum class SizeStrictness { Strict, Random };
...
public:
    ...
    std::string fillRandomTest( dev::test::TestSuite const& _testSuite,
        std::string const& _testFillerTemplate, test::RandomCodeOptions const& _options );

    /// Generate random byte string of a given length
    std::string rndByteSequence(
        int _length = 1, SizeStrictness _sizeType = SizeStrictness::Strict );
    ...
    /// Generate random
    std::string randomUniIntHex(
        u256 const& _minVal = 0, u256 const& _maxVal = std::numeric_limits< int64_t >::max() );
    virtual u256 randomUniInt(
        u256 const& _minVal = 0, u256 const& _maxVal = std::numeric_limits< int64_t >::max() ) = 0;

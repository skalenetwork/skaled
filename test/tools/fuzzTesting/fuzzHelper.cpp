...
const static std::array< eth::Instruction, 47 > invalidOpcodes{{eth::Instruction::INVALID,
    ...
    eth::Instruction::BEGINDATA, eth::Instruction::RETURNSUB, eth::Instruction::PUTLOCAL,
    eth::Instruction::GETLOCAL, eth::Instruction::XADD, eth::Instruction::XMUL,
    eth::Instruction::XSUB, eth::Instruction::XDIV, eth::Instruction::XSDIV, eth::Instruction::XMOD,
    eth::Instruction::XSMOD, eth::Instruction::XLT, eth::Instruction::XGT, eth::Instruction::XSLT,
    eth::Instruction::XSGT, eth::Instruction::XEQ, eth::Instruction::XISZERO,
    eth::Instruction::XAND, eth::Instruction::XOR, eth::Instruction::XXOR, eth::Instruction::XNOT,
    eth::Instruction::XSHL, eth::Instruction::XSHR, eth::Instruction::XSAR, eth::Instruction::XROL,
...
int RandomCodeBase::recursiveRLP( std::string& _result, int _depth, std::string& _debug ) {
    ...
    if ( _depth == 1 ) {
        ...
        case 0: {
            ...
            std::string rlp =
                emptyZeros +
                toCompactHex( genbug ? randomSmallUniInt() % 255 : randomSmallUniInt() % 128, 1 );
        ...
        case 1: {
            ...
            _debug.insert( 0,
                "[" + toCompactHex( 128 + len ) + "(" + toString( len ) + ")]" + emptyZeros + hex );
            return len + 1;
        }
        case 2: {
            ...
            _debug.insert( 0, "[" + toCompactHex( 183 + hexlen.size() / 2 ) + hexlen + "(" +
                                  toString( len ) + "){2}]" + emptyZeros + hex );
        ...
        case 3: {
            ...
            _debug.insert(
                0, "[" + toCompactHex( 192 + len ) + "(" + toString( len ) + "){3}]" + hex );
            return len + 1;
        }
        case 4: {
            ...
            if ( len < 56 && genValidRlp )
                len = 56;
            std::string hexlen = emptyZeros2 + toCompactHex( len, 1 );
            std::string rlpblock = toCompactHex( 247 + hexlen.size() / 2 ) + hexlen + emptyZeros +
                                   rndByteSequence( len );
            _debug.insert( 0, "[" + toCompactHex( 247 + hexlen.size() / 2 ) + hexlen + "(" +
                                  toString( len ) + "){4}]" + emptyZeros + rndByteSequence( len ) );
            _result.insert( 0, rlpblock );
            return rlpblock.size() / 2;
        }
        }
...
std::string RandomCodeBase::rndByteSequence( int _length, SizeStrictness _sizeType ) {
    std::string hash;
    _length = ( _sizeType == SizeStrictness::Strict ) ? std::max( 0, _length ) :
                                                        ( int ) randomUniInt( 0, _length );
    for ( auto i = 0; i < _length; i++ ) {
        uint8_t _byte_ = randomOpcode();
        hash += toCompactHex( _byte_, 1 );
    }
    return hash;
}
...
bool isOpcodeDefined( uint8_t _opcode ) {
    ...
    return (
        info.gasPriceTier != dev::eth::Tier::Invalid && info.name != nullptr &&
...
std::string RandomCodeBase::fillArguments(
    eth::Instruction _opcode, RandomCodeOptions const& _options ) {
    ...
    if ( rand < _options.smartCodeProbability )  // Smart.
    {
        ...
        case eth::Instruction::CALLCODE:
            ...
            code += getPushCode( toString( _options.getRandomAddress(
                RandomCodeOptions::AddressType::PrecompiledOrState ) ) );  // address
...
RandomCodeOptions::RandomCodeOptions()
    : useUndefinedOpCodes( false ),   // spawn undefined bytecodes in code
      smartCodeProbability( 99 ),     // spawn correct opcodes (with correct argument stack and
                                      // reasonable arguments)
      randomAddressProbability( 3 ),  // probability of generating a random address instead of
                                      // defined from list
      emptyCodeProbability( 2 ),  // probability of code being empty (empty code mean empty account)
      emptyAddressProbability( 15 ),  // probability of generating an empty address for transaction
                                      // creation
      precompiledAddressProbability( 5 ),  // probability of generating a precompiled address for
                                           // calls
      byzPrecompiledAddressProbability( 10 ),  // probability of generating a precompiled address
                                               // for calls
      precompiledDestProbability( 2 ),  // probability of generating a precompiled address as tx
                                        // destination
      sendingAddressProbability( 3 )    // probability of calling to the tx sending account
{
    ...
    for ( uint8_t i = static_cast< uint8_t >( eth::Instruction::PUSH1 );
          i <= static_cast< uint8_t >( eth::Instruction::PUSH32 ); i++ )
        setWeight( ( eth::Instruction ) i, 1 );
    for ( uint8_t i = static_cast< uint8_t >( eth::Instruction::SWAP1 );
          i <= static_cast< uint8_t >( eth::Instruction::SWAP16 ); i++ )
        setWeight( ( eth::Instruction ) i, 10 );
    for ( uint8_t i = static_cast< uint8_t >( eth::Instruction::DUP1 );
          i <= static_cast< uint8_t >( eth::Instruction::DUP16 ); i++ )
        setWeight( ( eth::Instruction ) i, 10 );
    ...
    addAddress(
        Address( "0xa94f5374fce5edbc8e2a8697c15331677e6ebf0b" ), AddressType::SendingAccount );
    addAddress(
        Address( "0xffffffffffffffffffffffffffffffffffffffff" ), AddressType::StateAccount );
    addAddress(
        Address( "0x1000000000000000000000000000000000000000" ), AddressType::StateAccount );
    addAddress(
        Address( "0xb94f5374fce5edbc8e2a8697c15331677e6ebf0b" ), AddressType::StateAccount );
    addAddress(
        Address( "0xc94f5374fce5edbc8e2a8697c15331677e6ebf0b" ), AddressType::StateAccount );
    addAddress(
        Address( "0xd94f5374fce5edbc8e2a8697c15331677e6ebf0b" ), AddressType::StateAccount );
    ...
    addAddress( Address( "0x0000000000000000000000000000000000000005" ),
        AddressType::ByzantiumPrecompiled );
...
void RandomCodeOptions::loadFromFile( boost::filesystem::path const& _jsonFileName ) {
    ...
    byzPrecompiledAddressProbability =
        getProbability( probObj.at( "byzPrecompiledAddressProbability" ) );
...
void RandomCodeOptions::addAddress( Address const& _address, AddressType _type ) {
    if ( _type != Precompiled && _type != ByzantiumPrecompiled && _type != StateAccount &&
         _type != SendingAccount )
        BOOST_ERROR( "RandomCodeOptions::addAddress: address type could not be added! " +
                     toString( _type ) );
...
Address RandomCodeOptions::getRandomAddressPriv( AddressType _type ) const {
    if ( _type != Precompiled && _type != ByzantiumPrecompiled && _type != StateAccount &&
         _type != SendingAccount )
        BOOST_ERROR( "RandomCodeOptions::getRandomAddressPriv: address type could not be found! " +
                     toString( _type ) );

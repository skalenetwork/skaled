...
#define ETH_CHECK_EQUAL( x, y )            \
    {                                      \
        dev::WriteGuard wg( x_boostTest ); \
        BOOST_CHECK_EQUAL( x, y );         \
    }
#define ETH_CHECK_EQUAL_COLLECTIONS( xb, xe, yb, ye )    \
    {                                                    \
        dev::WriteGuard wg( x_boostTest );               \
        BOOST_CHECK_EQUAL_COLLECTIONS( xb, xe, yb, ye ); \
    }
...
struct BlockChainFixture : public LoadTestFileFixture {
    void enumerateBlockchains( std::function< void(
            Json::Value const&, dev::eth::BlockChain const&, dev::eth::StateClass state ) >
            callback ) const;
};
...
struct ParallelClientBaseFixture : public ClientBaseFixture,
                                   public ParallelFixture,
                                   public TestOutputHelperFixture {
    void enumerateClients(
        std::function< void( Json::Value const&, dev::eth::ClientBase& ) > callback ) const;
};

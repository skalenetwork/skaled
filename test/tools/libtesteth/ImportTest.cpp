    ...
    for ( auto const& i : output ) {
        ...
        if ( value.type() == json_spirit::int_type )
            ...
            for ( auto const& j : value.get_array() ) {
                ...
                arr.push_back( ( str.substr( 0, 2 ) == "0x" ) ?
                                   str :
                                   toCompactHexPrefixed( toInt( str ), 1 ) );
            }
            output[key] = arr;
            continue;
        } else
...
int ImportTest::compareStatesFAIR( State const& _stateExpect, State const& _statePost,
    unordered_set< Address > const& owners, AccountMaskMap const _expectedStateOptions,
    WhenError _throw ) {
    bool wasError = false;
#define CHECK( a, b )                       \
    {                                       \
...
bool ImportTest::checkGeneralTestSectionSearch( json_spirit::mObject const& _expects,
    ...
    for ( size_t i = 0; i < lookTransactions.size(); i++ ) {
        ...
        if ( network.count( netIdToString( tr.netId ) ) || network.count( "ALL" ) )
            if ( ( inArray( d, tr.dataInd ) || d[0] == -1 ) &&
                 ( inArray( g, tr.gasInd ) || g[0] == -1 ) &&
...
#ifdef FAIR
                    int errcode = ImportTest::compareStatesFAIR( expectState, postState,
                        unordered_set< Address >(), stateMap, WhenError::Throw );
#else
                    int errcode = ImportTest::compareStates(
                        expectState, postState, stateMap, WhenError::Throw );
#endif
                    if ( errcode > 0 ) {
                        cerr << trInfo << "\n";
                    ...
                    if ( _expects.count( "logs" ) )
                        BOOST_CHECK_MESSAGE(
                            ...
                            TestOutputHelper::get().testName() + " on " +
                                ...
                                _expects.at( "logs" ).get_str() + " actual: " +
                                exportLog( tr.output.second.log() ) + " in " + trInfo );
                    else
                        BOOST_ERROR(
...
int ImportTest::exportTest() {
    ...
    for ( size_t i = 0; i < m_transactions.size(); i++ ) {
        ...
        obj["value"] = tr.valInd;
        obj2["indexes"] = obj;
        obj2["hash"] = toHexPrefixed( EmptySHA3.asBytes() );
        obj2["logs"] = exportLog( tr.output.second.log() );

        // Print the post state if transaction has failed on expect section
        auto it =
            std::find( std::begin( stateIndexesToPrint ), std::end( stateIndexesToPrint ), i );

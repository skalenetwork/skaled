...
void TestOutputHelper::showProgress() {
    ...
    if ( !test::Options::get().createRandomTest &&
         ( m_currTest % m_testsPerProgs == 0 || m_currTest == m_maxTests ) ) {
...
void TestOutputHelper::printTestExecStats() {
    if ( Options::get().exectimelog ) {
        ...
        std::cout << std::left;
        std::sort( m_execTimeResults.begin(), m_execTimeResults.end(),
            []( execTimeName _a, execTimeName _b ) { return ( _b.first < _a.first ); } );
        int totalTime = std::accumulate( m_execTimeResults.begin(), m_execTimeResults.end(), 0,
            []( int a, execTimeName const& b ) { return a + b.first; } );

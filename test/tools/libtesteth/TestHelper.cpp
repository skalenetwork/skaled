...
void mine( Client& _c, int _numBlocks ) {
    mineMoney( _c, _numBlocks );
}

// mines empty block, because doesn't use transaction queue
void mineMoney( Client& _c, int _numBlocks ) {
    int sealedBlocks = 0;
    ...
    auto importHandler = _c.setOnBlockImport(
        [_numBlocks, &importedBlocks, &allBlocksImported]( BlockHeader const& ) {
...
// runs transaction through consensus, does not get money
void mineTransaction( Client& _c, int _numBlocks ) {
    int sealedBlocks = 0;
    auto sealHandler = _c.setOnBlockSealed( [_numBlocks, &sealedBlocks, &_c]( bytes const& ) {
        if ( ++sealedBlocks == _numBlocks )
            _c.stopSealing();
...
eth::Network stringToNetId( string const& _netname ) {
    ...
    static vector< eth::Network > const networks{
        {eth::Network::FrontierTest, eth::Network::HomesteadTest, eth::Network::EIP150Test,
            ...
            eth::Network::EIP158ToByzantiumAt5, eth::Network::ByzantiumToConstantinopleAt5,
...
_byte_ toByte( json_spirit::mValue const& _v ) {
    switch ( _v.type() ) {
    case json_spirit::str_type:
        return ( _byte_ ) stoi( _v.get_str() );
    case json_spirit::int_type:
        return ( _byte_ ) _v.get_uint64();
    case json_spirit::bool_type:
        return ( _byte_ ) _v.get_bool();
    case json_spirit::real_type:
        return ( _byte_ ) _v.get_real();
    default:
        cwarn << "Bad type for scalar: " << _v.type();
    }
    return 0;

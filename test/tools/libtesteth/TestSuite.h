...
public:
    ...
    // Main test executive function. should be declared for each test suite. it fills and runs the
    // test .json file
    virtual json_spirit::mValue doTests( json_spirit::mValue const&, bool ) const = 0;

    // Execute all tests from suiteFolder()/_testFolder/*
    // This functions checks that tests in the repo are updated with
    // /src/suiteFillerFolder()/*Filler tests Then it looks for a Filler.json file in
    // /src/suiteFillerFolder()/_testFolder and generates the actual tests If src test file is named
    // Copier it will just copy it from /src folder because such test is crafted and it's RLP could
    // not be generated by the client If the src test does not end up with either Filler.json or
    // Copier.json an exception occurs.

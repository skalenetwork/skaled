...
public:
    ...
    virtual void setStore( u256 _n, u256 _v ) override {
        std::get< 2 >( addresses[myAddress] )[_n] = _v;
    }
    ...
    virtual size_t codeSizeAt( Address _a ) override {
        return std::get< 3 >( addresses[_a] ).size();
    }
    eth::CreateResult create( u256 _endowment, u256& io_gas, bytesConstRef _init,
        eth::Instruction _op, u256 _salt, eth::OnOpFunc const& ) override;
    ...
    void setContract( Address _myAddress, u256 _myBalance, u256 _myNonce,
        std::map< u256, u256 > const& _storage, bytes const& _code );
    void set( Address _a, u256 _myBalance, u256 _myNonce, std::map< u256, u256 > const& _storage,
        bytes const& _code );
    ...
    static dev::eth::EnvInfo importEnv(
        json_spirit::mObject const& _o, eth::LastBlockHashesFace const& _lastBlockHashes, time_t _latestBlockTimestamp );

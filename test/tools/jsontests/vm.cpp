...
h256 FakeExtVM::blockHash( u256 _number ) {
    ...
    if ( _number < envInfo().number() &&
         _number >= ( std::max< u256 >( 256, envInfo().number() ) - 256 ) )
        return sha3( toString( _number ) );

    return h256();
}

void FakeExtVM::set( Address _a, u256 _myBalance, u256 _myNonce, map< u256, u256 > const& _storage,
    bytes const& _code ) {
...
EnvInfo FakeExtVM::importEnv( mObject const& _o, LastBlockHashesFace const& _lastBlockHashes ) {
    ...
    blockHeader.setMicrosecondsExDifficulty( toInt( _o.at( "currentDifficulty" ) ) );
...
void FakeExtVM::importState( mObject const& _object ) {
    for ( auto const& i : _object ) {
        ...
        // cant use BOOST_REQUIRE, because this function is used outside boost test
        // (createRandomTest)
...
void FakeExtVM::importExec( mObject const& _o ) {
    ...
    if ( _o.count( "code" ) == 0 ||
         ( _o.at( "code" ).type() != str_type && _o.at( "code" ).type() != array_type ) )
        code.clear();
    ...
    for ( mValue const& v : _callcreates ) {
        ...
        Transaction t =
            tx["destination"].get_str().empty() ?
                Transaction( toInt( tx["value"] ), 0, toInt( tx["gasLimit"] ),
                    fromHex( tx["data"].get_str() ) ) :
                Transaction( toInt( tx["value"] ), 0, toInt( tx["gasLimit"] ),
...
json_spirit::mValue VmTestSuite::doTests( json_spirit::mValue const& _input, bool _fillin ) const {
    ...
    for ( auto& i : _input.get_obj() ) {
        ...
        json_spirit::mObject& testOutput =
            output[testname].get_obj();  // TODO: avoid copying and add valid fields one by one
        ...
        if ( _fillin ) {
            ...
            if ( vmExceptionOccured ) {
                ...
                BOOST_REQUIRE_MESSAGE( testOutput.at( "post" ).type() == obj_type,
                    testname + "fev.exportState returned something not an object." );
                ...
                BOOST_REQUIRE_MESSAGE( testOutput.count( "expect" ) == 0,
                    testname + " expect should never been added to the output!" );

                testOutput["callcreates"] = fev.exportCallCreates();
                testOutput["out"] = output.size() > 4096 ? "#" + toString( output.size() ) :
                                                           toHexPrefixed( output );

                // compare expected output with post output
                if ( testInput.count( "expectOut" ) > 0 ) {
                    BOOST_REQUIRE_MESSAGE( testInput.at( "expectOut" ).type() == str_type,
                        testname + " expectOut field is not a string." );
                    std::string warning =
                        " Check State: Error! Unexpected output: " + testOutput["out"].get_str() +
                        " Expected: " + testInput.at( "expectOut" ).get_str();
                    BOOST_CHECK_MESSAGE(
                        testOutput["out"].get_str() == testInput.at( "expectOut" ).get_str(),
                        warning );
                }
            ...
            {
                BOOST_REQUIRE_MESSAGE( testInput.at( "post" ).type() == obj_type,
                    testname + " post field is not an object." );
                BOOST_CHECK( !vmExceptionOccured );

                BOOST_REQUIRE_MESSAGE( testInput.count( "callcreates" ) > 0,
                    testname + " callcreates field is missing." );
                BOOST_REQUIRE_MESSAGE( testInput.at( "callcreates" ).type() == array_type,
                    testname + " callcreates field is not an array." );
                BOOST_REQUIRE_MESSAGE(
                    testInput.count( "out" ) > 0, testname + " out field is missing." );
                BOOST_REQUIRE_MESSAGE(
                    testInput.count( "gas" ) > 0, testname + " gas field is missing." );
                BOOST_REQUIRE_MESSAGE(
                    testInput.count( "logs" ) > 0, testname + " logs field is missing." );

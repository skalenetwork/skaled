...
mObject getExpectSection( mValue const& _expect, eth::Network _network ) {
    ...
    for ( auto const& value : _expect.get_array() ) {
        ...
        BOOST_REQUIRE_MESSAGE(
            obj.count( "network" ), "network section not set in expect section!" );
        set< string > networks;
        ImportTest::parseJsonStrValueIntoSet( obj["network"], networks );
        BOOST_CHECK_MESSAGE(
            networks.size() > 0, TestOutputHelper::get().testName() + " Network array not set!" );
        ImportTest::checkAllowedNetwork( networks );

        if ( networks.count( test::netIdToString( _network ) ) ||
             networks.count( string( "ALL" ) ) )
            objVector.push_back( obj );
    }
    BOOST_REQUIRE_MESSAGE( objVector.size() == 1,
        "Expect network should occur once in expect section of transaction test filler! (" +
            test::netIdToString( _network ) + ")" );
    return objVector.at( 0 );
}

json_spirit::mObject FillTransactionTest( json_spirit::mObject const& _o ) {
    mObject out;
    BOOST_REQUIRE_MESSAGE( _o.count( "transaction" ) > 0,
        "transaction section not found! " + TestOutputHelper::get().testFile().string() );
    BOOST_REQUIRE_MESSAGE( _o.count( "expect" ) > 0,
        "expect section not found! " + TestOutputHelper::get().testFile().string() );
    ...
    for ( auto const network : test::getNetworks() ) {
        ...
        try {
            ...
            if ( expectSection.count( "sender" ) > 0 ) {
                ...
                BOOST_CHECK_MESSAGE( toString( txFromFields.sender() ) == expectSender,
                    "Error filling transaction test " + TestOutputHelper::get().testName() +
                        ": expected another sender address! (got: " +
                        toString( txFromFields.sender() ) + "), expected: (" + expectSender + ")" );
            }
            ...
            BOOST_CHECK_MESSAGE(
                expectInValid, TestOutputHelper::get().testName() +
                                   " Check state: Transaction is expected to be valid! (" +
                                   test::netIdToString( network ) + ")" );
            continue;
        }
        ...
        BOOST_CHECK_MESSAGE(
            expectValid, TestOutputHelper::get().testName() +
                             " Check state: Transaction is expected to be invalid! (" +
                             test::netIdToString( network ) + ")" );
    }
    return out;
}
...
void TestTransactionTest( json_spirit::mObject const& _o ) {
    ...
    for ( auto const network : test::getNetworks() ) {
        ...
        BOOST_REQUIRE_MESSAGE( _o.count( networkname ) > 0,
            testname + " Transaction test missing network results! (" + networkname + ")" );
        ...
        BOOST_CHECK_MESSAGE( txFromRlp.sender() == addressExpected,
            testname + "Signature address of sender does not match given sender address! (" +
                networkname + ")" );

        BOOST_REQUIRE_MESSAGE( obj.count( "hash" ) > 0,
            testname + "Expected a valid transaction! (" + networkname + ")" );
        h256 txSha3Expected = h256( obj["hash"].get_str() );
        BOOST_CHECK_MESSAGE( txFromRlp.sha3() == txSha3Expected,
            testname + "Expected different transaction hash! (" + networkname + ")" );
    }
}

json_spirit::mValue TransactionTestSuite::doTests(
    json_spirit::mValue const& _input, bool _fillin ) const {
    BOOST_REQUIRE_MESSAGE(
        _input.type() == obj_type, TestOutputHelper::get().get().testFile().string() +
                                       " TransactionTest file should contain an object." );
    BOOST_REQUIRE_MESSAGE( !_fillin || _input.get_obj().size() == 1,
        TestOutputHelper::get().testFile().string() +
            " TransactionTest filler should contain only one test." );

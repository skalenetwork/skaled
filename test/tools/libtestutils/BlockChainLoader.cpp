...
BlockChainLoader::BlockChainLoader( Json::Value const& _json, eth::Network _sealEngineNetwork )
    ...
    Json::FastWriter a;
    std::shared_ptr< ChainParams > chainParams( new ChainParams( genesisInfo( _sealEngineNetwork ), genesisBlock,
                                        jsonToAccountMap( a.write( _json["pre"] ) ) ) );
    m_bc.reset( new BlockChain( chainParams, m_dir.path(), false, WithExisting::Kill ) );
    ...
    for ( auto const& block : _json["blocks"] ) {
        ...
        m_bc->import( rlp, m_block.mutableState() );
    }

    // sync state
    m_block.sync( *m_bc, m_block.mutableState() );
}

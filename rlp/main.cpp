/*
    Modifications Copyright (C) 2018-2019 SKALE Labs Authors
...
enum class Mode { AssembleArchive, ListArchive, ExtractArchive, Render, Create };
...
public:
    ...
    void output( RLP const& _d, unsigned _level = 0 ) {
        if ( _d.isNull() )
            ...
            int j = 0;
            for ( auto i : _d ) {
                m_out << ( j++ ? ( m_prefs.indent.empty() ? ", " : ( "," + newline ) ) :
...
int main( int argc, char** argv ) {
    ...
    addRenderOption( "ascii-strings",
        "Render data as C-style strings or hex depending on content being ASCII." );
    ...
    try {
        ...
        case Mode::AssembleArchive: {
            if ( boost::filesystem::is_directory( inputFile ) ) {
                js::mArray entries;
                auto basePath =
                    boost::filesystem::canonical( boost::filesystem::path( inputFile ) ).string();
                for ( string& i : otherInputs ) {
                    ...
                    if ( !entry.count( "path" ) ) {
                        std::string path =
                            boost::filesystem::canonical( boost::filesystem::path( parsed[0] ) )
                                .string()
                                .substr( basePath.size() );
                        if ( path == "/index.html" )
                            path = "/";
                        entry["path"] = path;
                    }

...
#define STATE_CHECK(_EXPRESSION_)                                                              \
    if ( !( _EXPRESSION_ ) ) {                                                                 \
        ...
        BOOST_THROW_EXCEPTION(std::runtime_error(__msg__)); \
    }
...
#endif
{
    ...
    m_levelDBSnap = _s.m_levelDBSnap;
...
std::pair<State::AddressMap, h256> State::addresses(
    ...
    if (addresses.size() > _maxResults) {
        STATE_CHECK(numeric_limits<long>::max() >= long(_maxResults));
...
void State::createAccount(Address const &_address, eth::Account const &&_account) {
    STATE_CHECK(!addressInUse(_address) && "Account already exists");
...
void State::setCode(Address const &_address, bytes &&_code, u256 const &_version) {
    ...
    STATE_CHECK(!addressHasCode(_address));
...
void State::clearCachesAndUpdateToLatestVersion() {
    m_changeLog.clear();
    m_cache.clear();
    m_unchangedCacheEntries.clear();
    ...
    if (m_levelDBSnap) {
        // we do not do any locks/version checks  in snaps
        return;
    }
...
State State::createStateReadOnlyCopy() const {
    ...
    stateCopy.clearCachesAndUpdateToLatestVersion();
    return stateCopy;
}

State State::createStateSnapCopy(std::shared_ptr<dev::db::LevelDBSnap> &_snap) const {
    State stateCopy = State(*this);
    stateCopy.useLevelDBSnap(_snap);
    return stateCopy;
}
...
State State::createNewCopyWithLocks() {
    ...
    copy.clearCachesAndUpdateToLatestVersion();
    return copy;
}
...
std::pair<ExecutionResult, TransactionReceipt> State::execute(EnvInfo const &_envInfo,
                                                              ...
                                                              Transaction const &_t, Permanence _p,
    ...
    if (m_levelDBSnap) {
        // snaps only support read only calls
        STATE_CHECK(_p == Permanence::Reverted);
    }
...
void State::useLevelDBSnap(std::shared_ptr<dev::db::LevelDBSnap> &_snap) {
    //STATE_CHECK(_snap);
    m_levelDBSnap = _snap;
}

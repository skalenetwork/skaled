    ...
    try {
        ...
        for ( size_t idxMessage = 0; idxMessage < cntMessagesToSign; ++idxMessage ) {
            ...
            if ( bIsVerifyImaMessagesViaLogsSearch ) {
                ...
                if ( jarrFoundLogRecords.is_array() && jarrFoundLogRecords.size() > 0 )
                    bIsVerified = true;
                if ( !bIsVerified ) {
                    cerror << DETAILED_ERROR throw std::runtime_error(
                        "IMA message " + std::to_string( nStartMessageIdx + idxMessage ) +
                ...
                for ( idxFoundLogRecord = 0; idxFoundLogRecord < cntFoundLogRecords;
                    ...
                    try {
                        ...
                        cerror << DETAILED_ERROR clog( VerbosityError, "IMA" )
        ...
        cerror << DETAILED_ERROR clog( VerbosityError, "IMA" )
               << ( strLogPrefix + " " + cc::fatal( "FATAL:" ) +
                      ...
                      cc::info( "IMA Verify and Sign" ) + cc::error( ", exception information: " ) +
                      cc::warn( ex.what() ) );
        throw jsonrpc::JsonRpcException( exceptionToErrorMessage() );
    } catch ( const std::exception& ex ) {
        cerror << DETAILED_ERROR clog( VerbosityError, "IMA" )
...
Json::Value SkaleStats::skale_imaBSU256( const Json::Value& request ) {
    ...
    try {
        ...
        cerror << DETAILED_ERROR throw jsonrpc::JsonRpcException( exceptionToErrorMessage() );
    } catch ( const std::exception& ex ) {
        clog( VerbosityError, "IMA" )
            << ( strLogPrefix + " " + cc::fatal( "FATAL:" ) +
                   cc::error( " Exception while processing " ) + cc::info( "IMA BLS Sign U256" ) +
                   cc::error( ", exception information: " ) + cc::warn( ex.what() ) );
        cerror << DETAILED_ERROR throw jsonrpc::JsonRpcException( ex.what() );
    } catch ( ... ) {
        clog( VerbosityError, "IMA" )
            << ( strLogPrefix + " " + cc::fatal( "FATAL:" ) +
                   cc::error( " Exception while processing " ) + cc::info( "IMA BLS Sign U256" ) +
                   cc::error( ", exception information: " ) + cc::warn( "unknown exception" ) );
        cerror << DETAILED_ERROR throw jsonrpc::JsonRpcException( "unknown exception" );
    }
}  // skale_imaBSU256()

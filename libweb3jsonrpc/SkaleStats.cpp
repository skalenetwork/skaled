...
Json::Value SkaleStats::skale_imaVerifyAndSign( const Json::Value& request ) {
    ...
    try {
        ...
        for ( size_t idxMessage = 0; idxMessage < cntMessagesToSign; ++idxMessage ) {
            ...
            //                //
            //                //
            //                // Forming eth_getLogs query similar to web3's getPastEvents, see
            //                details here:
            ...
            //                std::cout << strLogPrefix << cc::debug( " Will execute logs search
            //                query: " )
            //                          << cc::j( joLogsQuery ) << "\n";
            //                //
            //                //
            ...
            //                    if ( d.empty() )
            //                        throw std::runtime_error( "Main Net call to eth_getLogs
            //                        failed" );
            ...
            //                        this->client()->logs( toLogFilter( jvLogsQuery,
            //                        *this->client() ) ) );
            //                    jarrFoundLogRecords =
            //                        nlohmann::json::parse( Json::FastWriter().write( jvLogs ) );
            //                }  // else from if( strDirection == "M2S" )
            //                std::cout << strLogPrefix << cc::debug( " Got logs search query
            //                result: " )
            ...
            //                                              std::to_string( nStartMessageIdx +
            //                                              idxMessage ) + " verification failed -
            //                                              not found in logs" );
            //                //
            //                //
            ...
            //                size_t idxFoundLogRecord = 0, cntFoundLogRecords =
            //                jarrFoundLogRecords.size(); for ( idxFoundLogRecord = 0;
            //                idxFoundLogRecord < cntFoundLogRecords;
            //                      ++idxFoundLogRecord ) {
            //                    const nlohmann::json& joFoundLogRecord =
            //                    jarrFoundLogRecords[idxFoundLogRecord]; if (
            //                    joFoundLogRecord.count( "transactionHash" ) == 0 )
            //                        continue;  // bad log record??? this should never happen
            //                    const nlohmann::json& joTransactionHash =
            //                    joFoundLogRecord["transactionHash"]; if (
            //                    !joTransactionHash.is_string() )
            //                        continue;  // bad log record??? this should never happen
            //                    const std::string strTransactionHash = joTransactionHash.get<
            //                    std::string >(); if ( strTransactionHash.empty() )
            //                        continue;  // bad log record??? this should never happen
            ...
            //                            else
            //                                jvTransaction = toJson(
            //                                this->client()->localisedTransaction( h ) );
            //                            joTransaction =
            //                                nlohmann::json::parse( Json::FastWriter().write(
            //                                jvTransaction ) );
            //                        }  // else from if ( strDirection == "M2S" )
            //                    } catch ( const std::exception& ex ) {
            ...
            //                    // extract "to" address from transaction, then compare it with
            //                    "sender" from IMA
            //                    // message
            //                    const std::string strTransactionTo = skutils::tools::trim_copy(
            //                        ( joTransaction.count( "to" ) > 0 &&
            //                        joTransaction["to"].is_string() ) ?
            ...
            //                            if ( d.empty() )
            //                                throw std::runtime_error(
            //                                    "Main Net call to eth_getTransactionReceipt
            //                                    failed" );
            //                            joTransactionReceipt = nlohmann::json::parse( d.s_
            //                            )["result"];
            //                        } else {
            ...
            //                        continue;  // ???
            //                    const nlohmann::json& jarrLogsReceipt =
            //                    joTransactionReceipt["logs"]; if ( !jarrLogsReceipt.is_array() )
            //                        continue;  // ???
            //                    bool bReceiptVerified = false;
            //                    size_t idxReceiptLogRecord = 0, cntReceiptLogRecords =
            //                    jarrLogsReceipt.size(); for ( idxReceiptLogRecord = 0;
            //                    idxReceiptLogRecord < cntReceiptLogRecords;
            //                          ++idxReceiptLogRecord ) {
            ...
            //                            continue;
            //                        bool bTopicSignatureFound = false, bTopicMsgCounterFound =
            //                        false,
            //                             bTopicDstChainHashFound = false;
            //                        const nlohmann::json& jarrReceiptTopics =
            //                        joReceiptLogRecord["topics"]; size_t idxReceiptTopic = 0,
            //                        cntReceiptTopics = jarrReceiptTopics.size(); for (
            //                        idxReceiptTopic = 0; idxReceiptTopic < cntReceiptTopics;
            //                              ++idxReceiptTopic ) {
            ...
            //                            continue;
            //                        const std::string strData = joReceiptLogRecord["data"].get<
            //                        std::string >(); if ( strData.empty() )
            //                            continue;
            //                        const std::string strDataLC_linear =
            //                        skutils::tools::trim_copy(
            //                            skutils::tools::replace_all_copy(
            //                            skutils::tools::to_lower( strData ),
            ...
            //                            }
            //                        }
            //                        if ( setChunksU256.find( uDestinationContract ) ==
            //                        setChunksU256.end() )
            //                            continue;
            //                        if ( setChunksU256.find( uDestinationAddressTo ) ==
            //                        setChunksU256.end() )
            //                            continue;
            //                        if ( setChunksU256.find( uMessageAmount ) ==
            //                        setChunksU256.end() )
            //                            continue;
            ...
            //                                  << cc::debug( " because no appropriate receipt was
            //                                  found" )
            //                                  << "\n";
            //                        continue;
            //                    }
            ...
            //                              cc::error( "." )
            //                              << "\n";
            //                    throw std::runtime_error( "No transaction was found for IMA
            //                    message " +
            //                                              std::to_string( nStartMessageIdx +
            //                                              idxMessage ) );
            //                }

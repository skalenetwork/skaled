...
template < class S >
bool IpcServerBase< S >::StartListening() {
...
template < class S >
bool IpcServerBase< S >::StopListening() {
...
template < class S >
void IpcServerBase< S >::GenerateResponse( S _connection ) {
    ...
    do {
        ...
        MICROPROFILE_SCOPEI( "IpcServerBase", "Read", MP_LAVENDER );
...
template < class S >
bool IpcServerBase< S >::OnRequest(
    const std::string& request, void* addInfo ) {  // l_sergiy: migration to new json-rpc-cpp
    string response;
    // if ( this->handler != NULL ) {
    ProcessRequest( request, response );  // this->handler->HandleRequest( request, response );

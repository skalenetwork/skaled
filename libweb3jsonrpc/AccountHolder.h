...
public:
    ...
    bool isRealAccount( Address const& _account ) const {
        return realAccounts().count( _account ) > 0;
    }
    ...
    virtual bool unlockAccount(
        Address const& /*_account*/, std::string const& /*_password*/, unsigned /*_duration*/
    ) {
        return false;
    }
...
public:
    SimpleAccountHolder( std::function< Interface*() > const& _client,
        std::function< std::string( Address ) > const& _getPassword, KeyManager& _keyman,
        std::function< bool( TransactionSkeleton const&, bool ) > _getAuthorisation =
            std::function< bool( TransactionSkeleton const&, bool ) >() )
    ...
    bool unlockAccount(
        Address const& _account, std::string const& _password, unsigned _duration ) override;

private:
    std::function< std::string( Address ) > m_getPassword;
    std::function< bool( TransactionSkeleton const&, bool ) > m_getAuthorisation;
    KeyManager& m_keyManager;
    std::map< Address, std::pair< std::chrono::steady_clock::time_point, unsigned > >
        m_unlockedAccounts;
};
...
public:
    FixedAccountHolder(
        std::function< Interface*() > const& _client, std::vector< dev::KeyPair > const& _accounts )
        : AccountHolder( _client ) {

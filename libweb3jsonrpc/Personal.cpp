...
Personal::Personal( KeyManager& _keyManager, AccountHolder& _accountHolder, eth::Interface& _eth )
    : m_keyManager( _keyManager ), m_accountHolder( _accountHolder ), m_eth( _eth ) {}
...
string Personal::personal_sendTransaction(
    ...
    if ( Secret s = m_keyManager.secret( t.from, [&]() { return _password; }, false ) ) {
        ...
        try {
            ...
            BOOST_THROW_EXCEPTION( JsonRpcException(
                std::string( "exception while submitting transaction: " ) + ex.what() ) );
        } catch ( ... ) {
            BOOST_THROW_EXCEPTION(
                JsonRpcException( "unknown exception while submitting transaction" ) );
        }
    }
...
bool Personal::personal_unlockAccount(
    std::string const& _address, std::string const& _password, int _duration ) {
    return m_accountHolder.unlockAccount(
        Address( fromHex( _address, WhenError::Throw ) ), _password, _duration );
}

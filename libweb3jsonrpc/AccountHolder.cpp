...
int AccountHolder::addProxyAccount( const Address& _account ) {
    ...
    id = int( u256( FixedHash< 32 >(
        sha3( bytesConstRef( ( _byte_* ) ( &id ), sizeof( int ) / sizeof( _byte_ ) ) ) ) ) );
...
bool SimpleAccountHolder::unlockAccount(
    Address const& _account, string const& _password, unsigned _duration ) {

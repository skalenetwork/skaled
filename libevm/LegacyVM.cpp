...
uint64_t LegacyVM::decodeJumpDest( const _byte_* const _code, uint64_t& _pc ) {
...
void LegacyVM::logGasMem() {
    ...
    m_runGas = toInt63( m_schedule->logGas + m_schedule->logTopicGas * n +
...
void LegacyVM::interpretCases() {
    ...
    DO_CASES {
        ...
        CASE( EXP ) {
            u256 expon = m_SP[1];
            m_runGas =
                toInt63( m_schedule->expGas +
                         m_schedule->expByteGas * ( 32 - ( h256( expon ).firstBitSet() / 8 ) ) );
            ON_OP();
            updateIOGas();
        ...
        CASE( BYTE ) {
            ...
            m_SPP[0] =
                m_SP[0] < 32 ? ( m_SP[1] >> ( unsigned ) ( 8 * ( 31 - m_SP[0] ) ) ) & 0xff : 0;
        }
        NEXT
        ...
        CASE( CALLDATALOAD ) {
            ...
            if ( u512( m_SP[0] ) + 31 < m_ext->data.size() )
                ...
                h256 r;
                for ( uint64_t i = ( uint64_t ) m_SP[0], e = ( uint64_t ) m_SP[0] + ( uint64_t ) 32,
                               j = 0;
                      i < e; ++i, ++j )
        ...
        CASE( SWAP1 ) CASE( SWAP2 ) CASE( SWAP3 ) CASE( SWAP4 ) CASE( SWAP5 ) CASE( SWAP6 )

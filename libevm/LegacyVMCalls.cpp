...
bool LegacyVM::caseCallSetup( CallParameters* callParams, bytesRef& o_output ) {
    ...
    if ( m_ext->balance( m_ext->myAddress ) >= callParams->valueTransfer && m_ext->depth < 1024 ) {
        ...
        callParams->senderAddress =
            m_OP == Instruction::DELEGATECALL ? m_ext->caller : m_ext->myAddress;
        callParams->receiveAddress =
            ( m_OP == Instruction::CALL || m_OP == Instruction::STATICCALL ) ?
                callParams->codeAddress :
                m_ext->myAddress;

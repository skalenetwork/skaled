...
evmc::bytes32 EvmCHost::get_storage(
    evmc::address const& _addr, evmc::bytes32 const& _key ) noexcept {
    ( void ) _addr;
...
size_t EvmCHost::copy_code( evmc::address const& _addr, size_t _codeOffset, _byte_* _bufferData,
...
void EvmCHost::selfdestruct(
    evmc::address const& _addr, evmc::address const& _beneficiary ) noexcept {
    ( void ) _addr;
    assert( fromEvmC( _addr ) == m_extVM.myAddress );

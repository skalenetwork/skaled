...
static js::mValue upgraded( string const& _s ) {
    ...
    if ( version == 1 ) {
        ...
        c["kdf"] = old["Crypto"].get_obj()["KeyHeader"].get_obj()["Kdf"];
        {
            ...
            for ( auto const& i :
                old["Crypto"].get_obj()["KeyHeader"].get_obj()["KdfParams"].get_obj() )
                if ( i.first != "SaltLen" )
...
bytesSec SecretStore::secret(
    h128 const& _uuid, function< string() > const& _pass, bool _useCache ) const {
...
bool SecretStore::recode(
    Address const& _address, string const& _newPass, function< string() > const& _pass, KDF _kdf ) {
    if ( auto k = key( _address ) ) {
...
pair< h128 const, SecretStore::EncryptedKey > const* SecretStore::key(
...
bool SecretStore::recode(
    h128 const& _uuid, string const& _newPass, function< string() > const& _pass, KDF _kdf ) {
...
string SecretStore::encrypt( bytesConstRef _v, string const& _pass, KDF _kdf ) {
    ...
    if ( derivedKey.empty() )
        BOOST_THROW_EXCEPTION(
            crypto::CryptoException() << errinfo_comment( "Key derivation failed." ) );
    ...
    if ( cipherText.empty() )
        BOOST_THROW_EXCEPTION(
            crypto::CryptoException() << errinfo_comment( "Key encryption failed." ) );
...
bytesSec SecretStore::decrypt( string const& _v, string const& _pass ) {
    ...
    if ( o["kdf"].get_str() == "pbkdf2" ) {
        ...
        derivedKey = scrypt( _pass, fromHex( p["salt"].get_str() ), p["n"].get_int(),
            p["r"].get_int(), p["p"].get_int(), p["dklen"].get_int() );
    } else {
    ...
    if ( o.count( "mac" ) ) {
        ...
        if ( o.count( "compat" ) && o["compat"].get_str() == "2" )
            macExp =
                sha3( derivedKey.ref().cropped( derivedKey.size() - 16 ).toBytes() + cipherText );
        else
            macExp = sha3( derivedKey.ref().cropped( 16, 16 ).toBytes() + cipherText );
        if ( mac != macExp ) {
            cwarn << "Invalid key - MAC mismatch; expected" << toString( macExp ) << ", got"
                  << toString( mac );
            return bytesSec();
        }
        ...
        h256 macExp =
            sha3( asBytes( o["sillymacjson"].get_str() ) +
                  derivedKey.ref().cropped( derivedKey.size() - 16 ).toBytes() + cipherText );
        if ( mac != macExp ) {
            cwarn << "Invalid key - MAC mismatch; expected" << toString( macExp ) << ", got"
                  << toString( mac );
            return bytesSec();
    ...
    if ( o["cipher"].get_str() == "aes-128-ctr" ) {
        ...
        if ( o.count( "compat" ) && o["compat"].get_str() == "2" ) {
            SecureFixedHash< 16 > key(
                sha3Secure( derivedKey.ref().cropped( derivedKey.size() - 16 ) ),
                h128::AlignRight );
            return decryptSymNoAuth( key, iv, &cipherText );
        } else
            return decryptSymNoAuth(
                SecureFixedHash< 16 >( derivedKey, h128::AlignLeft ), iv, &cipherText );
    } else {

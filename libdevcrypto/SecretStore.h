...
public:
    ...
    bytesSec secret( h128 const& _uuid, std::function< std::string() > const& _pass,
        bool _useCache = true ) const;
    /// @returns the secret key stored by the given @a _uuid.
    /// @param _pass function that returns the password for the key.
    ...
    /// Imports the (encrypted) key stored in the file @a _file and copies it to the managed
    /// directory.
    h128 importKey( std::string const& _file ) {
        auto ret = readKey( _file, false );
        if ( ret )
            save();
        return ret;
    }
    ...
    bool recode( h128 const& _uuid, std::string const& _newPass,
        std::function< std::string() > const& _pass, KDF _kdf = KDF::Scrypt );
    /// Decrypts and re-encrypts the key identified by @a _address.
    bool recode( Address const& _address, std::string const& _newPass,
        std::function< std::string() > const& _pass, KDF _kdf = KDF::Scrypt );
    ...
    h128 readKeyContent( std::string const& _content,
    ...
    /// @returns true if the current file @arg _uuid contains an empty address. m_keys will be
    /// updated with the given @arg _address.
    ...
    static boost::filesystem::path defaultPath() {
        return getDataDir( "web3" ) / boost::filesystem::path( "keys" );
    }
...
private:
    ...
    static std::string encrypt(
        bytesConstRef _v, std::string const& _pass, KDF _kdf = KDF::Scrypt );

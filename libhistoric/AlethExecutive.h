 ...
 * Example:
 * @code
 * AlethExecutive e(state, blockchain, 0);
...
public:
    ...
    AlethExecutive(dev::eth::HistoricState& io_s, Block const& _block, unsigned _txIndex, BlockChain
    const& _bc, unsigned _level = 0);
     */
...
private:
    ...
    dev::eth::HistoricState& m_s;  ///< The state to which this operation/transaction is applied.
    ...
    std::shared_ptr< AlethExtVM > m_ext;  ///< The VM externality object for the VM execution or
                                          ///< null if no VM is required. shared_ptr used only to
                                          ///< allow ExtVM forward reference. This field does *NOT*
                                          ///< survive this object.

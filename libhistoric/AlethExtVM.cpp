...
/// On what depth execution should be offloaded to additional separated stack space.
    ...
    void goOnOffloadedStack(AlethExecutive& _e, OnOpFunc const& _onOp)
    {
    ...
    void go(unsigned _depth, AlethExecutive& _e, OnOpFunc const& _onOp)
    {
...
size_t AlethExtVM::codeSizeAt(Address _a)
{
    return m_s.codeSize(_a);
}

h256 AlethExtVM::codeHashAt(Address _a)
{
    return exists(_a) ? m_s.codeHash(_a) : h256{};
}

void AlethExtVM::setStore(u256 _n, u256 _v)
{
    m_s.setStorage(myAddress, _n, _v);
}

CreateResult AlethExtVM::create(u256 _endowment, u256& io_gas, bytesConstRef _code, Instruction _op, u256 _salt, OnOpFunc const& _onOp)
{
...
h256 AlethExtVM::blockHash(u256 _number)
{
    ...
    ExecutionResult res;
    std::tie(res, std::ignore) = m_s.execute(envInfo(), m_sealEngine, tx, skale::Permanence::Reverted);

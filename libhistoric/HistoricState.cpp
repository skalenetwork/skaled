...
HistoricState::HistoricState(u256 const &_accountStartNonce, OverlayDB const &_db, OverlayDB const &_blockToStateRootDB, skale::BaseState _bs) :
        m_db(_db),
...
OverlayDB HistoricState::openDB(fs::path const &_basePath, h256 const &_genesisHash, WithExisting _we) {
...
void HistoricState::clearCacheIfTooLarge() const {
    ...
    while (m_unchangedCacheEntries.size() > 1000) {
        ...
        size_t const randomIndex = std::uniform_int_distribution<size_t>(0, m_unchangedCacheEntries.size() - 1)(
...
HistoricState::execute(EnvInfo const &_envInfo, SealEngineFace const &_sealEngine, Transaction const &_t, skale::Permanence _p,
        ...
        if (cache && !cache->isAlive())
            ...
            if ((cache && cache->codeHash() == EmptySHA3) || (!cache && r && (h256) r[3] != EmptySHA3)) {
                ...
                for (auto const &j: mem)
                    if (j.second)
                        contout << std::endl
                                << (delta.count(j.first) ? back.count(j.first) ? " *     " : " +     " : cached.count(
                                        j.first) ? " .     " : "       ") << std::hex << nouppercase << std::setw(64)
                                << j.first << ": " << std::setw(0) << j.second;
                    else
                        contout << std::endl << "XXX    " << std::hex << nouppercase << std::setw(64) << j.first << "";
            } else
                contout << " [SIMPLE]";
            _out << lead << i << ": " << std::dec << (cache ? cache->nonce() : r[0].toInt<u256>()) << " #:"
                 << (cache ? cache->balance() : r[1].toInt<u256>()) << contout.str() << std::endl;
        }
    }
    return _out;
}
...
/*HistoricState &dev::eth::createIntermediateState(HistoricState &o_s, Block const &_block, unsigned _txIndex, BlockChain const &_bc) {
...
dev::eth::commit<StateCacheDB>(AccountMap const &_cache, SecureTrieDB<Address, StateCacheDB> &_state);

...
namespace skutils {
	namespace command_line {
		...
		parser::parser(
			...
			, strDefaultCategoryName_( ( strDefaultCategoryName != nullptr && strDefaultCategoryName[0] != '\0' ) ? strDefaultCategoryName : "Other" ) // default is "Other", should not be empty
		...
		bool parser::empty() const {
			return ( args_.empty() || ( flag_handlers_.empty() && value_handlers_.empty() && (!default_handler_) ) ) ? true : false;
		}
		...
		bool parser::describe( const char * name, const char * desc ) { // describes only existing
			...
			if( ! ( 1 <= cntParts && cntParts <= 2 ) )
				return false;
			std::string /*strCategory( ( cntParts == 2 ) ? listParts.front() : strDefaultCategoryName_ ),*/ strName( listParts.back() );
		...
		std::string parser::description( const char * name ) const {
			...
			if( ! ( 1 <= cntParts && cntParts <= 2 ) )
				return desc;
			std::string /*strCategory( ( cntParts == 2 ) ? listParts.front() : strDefaultCategoryName_ ),*/ strName( listParts.back() );
			map_descriptions_t::const_iterator itFind = map_descriptions_.find( strName ), itEnd = map_descriptions_.cend();
			if( itFind == itEnd )
				return desc;
			desc = itFind->second;
			return desc;
		...
		bool parser::have_flag_handler( const char * name ) const {
			map_flag_handlers_t::const_iterator itFind = flag_handlers_.find( name ), itEnd = flag_handlers_.cend();
			if( itFind == itEnd )
				return false;
		...
		bool parser::have_value_handler( const char * name ) const {
			map_value_handlers_t::const_iterator itFind = value_handlers_.find( name ), itEnd = value_handlers_.cend();
			if( itFind == itEnd )
				return false;
		...
		std::string parser::options_text(
			...
			if( eoptt_effective == e_options_publishing_type_t::eoptt_auto ) {
				...
				else if( cntCategories == 1 && map_category_opts_.find(strDefaultCategoryName_) != map_category_opts_.end() )
					eoptt_effective = e_options_publishing_type_t::eoptt_linear;
				else
					eoptt_effective = e_options_publishing_type_t::eoptt_categorized;
			}
			static const char option_space_prefix[] = "  ", option_space_suffix[] = "...", value_suffix1[] = "=", value_suffix2[] = "<value>";
			size_t name_len_max = 0;
			std::for_each( list_sequence_handlers_.cbegin(), list_sequence_handlers_.cend(), [&]( name_sequence_list_t::const_reference name ) {
			...
			if( eoptt_effective == e_options_publishing_type_t::eoptt_linear ) {
				...
				std::for_each( list_sequence_categories_.cbegin(), list_sequence_categories_.cend(), [&]( name_sequence_list_t::const_reference strCategory ) {
					s += cc::sunny(strCategory) + cc::note(" options:") + "\n";
					const name_sequence_list_t & cat_opts = ( const_cast < parser * > ( this ) )->map_category_opts_[ strCategory ];
					std::for_each( cat_opts.cbegin(), cat_opts.cend(), [&]( name_sequence_list_t::const_reference name ) {
						std::string nameXnb = "--" + name;
						std::string nameX = cc::debug("--") + cc::note(name);
						if( const_cast < parser * > ( this ) -> value_handler(name) ) {
							nameXnb += value_suffix1;
		...
		bool parser::flag_handler( const char * name, const char * desc, parser::flag_argument_handler_t fn ) {
			if( name == nullptr || name[0] == '\0' )
				return false;
			if( ! fn )
				return false;
			...
			name_sequence_list_t::const_iterator
				itCatFind = std::find( list_sequence_categories_.cbegin(), list_sequence_categories_.cend(), strCategory ),
		...
		bool parser::value_handler( const char * name, const char * desc, parser::string_argument_value_handler_t fn ) {
			if( name == nullptr || name[0] == '\0' )
				return false;
			if( ! fn )
				return false;
		...
		bool parser::remove_entry( const char * name ) {
			...
			name_sequence_list_t & cat_opts = map_category_opts_[ strCategory ];
			name_sequence_list_t::iterator
				itNameFind = std::find( cat_opts.begin(), cat_opts.end(), strName ),
		...
		void parser::stat_network_interfaces_info( std::ostream & os ) {
			...
			os << cc::info("Number of of found IPv4 interfaces") + cc::debug("...................") << cc::size10(lst.size()) << std::endl;
			for( itWalk = lst.cbegin(), itEnd = lst.cend(); itWalk != itEnd; ++ itWalk )
				os << cc::debug("....") << cc::info("IPv4 interface") + cc::debug("...................................") << stat_gen_if(*itWalk) << std::endl;
			lst = skutils::network::get_machine_ip_addresses( false, true ); // try IP6
			os << cc::info("Number of of found IPv6 interfaces") + cc::debug("...................") << cc::size10(lst.size()) << std::endl;
			for( itWalk = lst.cbegin(), itEnd = lst.cend(); itWalk != itEnd; ++ itWalk )
				os << cc::debug("....") << cc::info("IPv6 interface") + cc::debug("...................................") << stat_gen_if(*itWalk) << std::endl;
		}

...
namespace skutils {
    ...
    std::list<std::string> split2list( const std::string & s, char delim, size_t nMaxSplits /*= std::string::npos*/ ) {
    ...
    std::vector<std::string> split2vector( const std::string & s, char delim, size_t nMaxSplits /*= std::string::npos*/ ) {
    ...
    static const uint8_t g_tbl[256] = {
   ...
   bool is_chars( const char * s, const char * e, std::uint8_t mask ) {
    ...
    std::string decode( const char * s, const char * e ) {
        ...
        while( s != e ) {
            char c = *s++, a, b;
            if( c=='%' ) {
                if( s == e || (a=stat_get_hex_digit(*s++)) < 0 || s==e || (b=stat_get_hex_digit(*s++)) < 0 )
    ...
    url & url::scheme( const std::string & s ) {
        ...
        if( o != strScheme_ ) {
            strScheme_ = o;
            wasBuilt_ = false;
            if( ( strScheme_ == "http" && strPort_ == "80" ) || ( strScheme_ == "https" && strPort_ == "443" ) )
                strPort_ = "";
        }
        return (*this);
    }
    ...
    std::pair<std::string,std::string> url::user_name_and_password() const { // returns strings pair object, not referemnce
    ...
    url & url::user_name_and_password( const std::pair<std::string,std::string> & n_p, bool bRemoveIfEmpty ) {
        std::string s;
        if( ! n_p.first.empty() ) {
            s += n_p.first;
            s += ':';
    ...
    url & url::user_name_and_password( const std::string & strName, const std::string & strPassword, bool bRemoveIfEmpty ) {
        return user_name_and_password( std::pair<std::string,std::string>( strName, strPassword ), bRemoveIfEmpty );
    }
                ...
                if( *b == '[' ) {
                    ...
                    if( *b == 'v' ) {
                        if( is_heximal_digit(*++b) ) {
                            ...
                            if( is_heximal_digit(*++b) ) {
                                ip_v=static_cast<int8_t>((ip_v<<8) | stat_get_hex_digit(*b));
                            }
                        }
    ...
    void url::compose_url() const {
        ...
        if( ! strHost_.empty() ) {
            ...
            if( ! strPath_.empty() ) {
                const char *b = strPath_.data(), *e = b + strPath_.length(), *p = find_first_of( b, e, ":/" );
                if( p != e && *p == ':' )
                    throw url::compose_error( "the first segment of the relative path can't contain ':'" );
            }
        }
        if( ! strPath_.empty() ) {
    ...
    std::ostream & url::output( std::ostream & o ) const {
        lazy_parse();
        if( ! wasBuilt_ ) compose_url();
        o << "Url:{url(" << strUrl_ << ")";
        if( ! strScheme_.empty() ) o << " scheme(" << strScheme_ << ")";
        if( ! strUserName_.empty() ) o << " user_info(" << strUserName_ << ")";
        if( nIPversion_ != -1 ) o << " host(" << strHost_ << ") IPv(" << (int)nIPversion_ << ")";
        if( ! strPort_.empty() ) o << " port(" << strPort_ << ")";
        if( ! strPath_.empty() ) o << " path(" << strPath_ << ")";
        if( ! kvQuery_.empty() ) {
            std::stringstream ss;
            ss << " query(";
        ...
        if( ! strFragment_.empty() ) o << "fragment(" << strFragment_ << ") ";
        o << "}";
        return o;
    }
    ...
    bool url::is_local_private_network_address() const {
        ...
        switch( ip_v ) {
        case 4:  return is_local_private_network_address_ipv4( host() );
        case 6:  return is_local_private_network_address_ipv6( host() );
        default: return false;
        } // switch( ip_v )
    }

    bool url::update_user_name_and_password( const char * strOptionalUser, const char * strOptionalPassword, bool bRemoveIfEmpty ) { // returns true if was updated
    ...
    bool url::update_user_name_and_password( const std::string & strOptionalUser, const std::string & strOptionalPassword, bool bRemoveIfEmpty ) { // returns true if was updated
        if( (!bRemoveIfEmpty) && strOptionalUser.empty() )
            return false;
        return update_user_name_and_password( strOptionalUser.c_str(), (!strOptionalPassword.empty()) ? strOptionalPassword.c_str() : nullptr, bRemoveIfEmpty );
    }
    ...
    bool update_url_user_name_and_password( url & an_url, const char * strOptionalUser, const char * strOptionalPassword, bool bRemoveIfEmpty ) {
        return an_url.update_user_name_and_password( strOptionalUser, strOptionalPassword, bRemoveIfEmpty );
    }
    bool update_url_user_name_and_password( url & an_url, const std::string & strOptionalUser, const std::string & strOptionalPassword, bool bRemoveIfEmpty ) {
        try {
            if( (!bRemoveIfEmpty) && strOptionalUser.empty() )
                return false;
            return update_url_user_name_and_password( an_url, strOptionalUser.c_str(), (!strOptionalPassword.empty()) ? strOptionalPassword.c_str() : nullptr, bRemoveIfEmpty );
        } catch( ... ) {
            return false;
        }
    }
    bool update_url_user_name_and_password( std::string & str_url, const char * strOptionalUser, const char * strOptionalPassword, bool bRemoveIfEmpty ) {
        try {
            url an_url( str_url );
            if( ! an_url.update_user_name_and_password( strOptionalUser, strOptionalPassword, bRemoveIfEmpty ) )
                return false;
    ...
    bool update_url_user_name_and_password( std::string & str_url, const std::string & strOptionalUser, const std::string & strOptionalPassword, bool bRemoveIfEmpty ) {
        if( (!bRemoveIfEmpty) && strOptionalUser.empty() )
            return false;
        return update_url_user_name_and_password( str_url, strOptionalUser.c_str(), (!strOptionalPassword.empty()) ? strOptionalPassword.c_str() : nullptr, bRemoveIfEmpty );
    }
    ...
    std::string update_url_user_name_and_password_copy( const std::string & str_url, const char * strOptionalUser, const char * strOptionalPassword, bool bRemoveIfEmpty ) {
        std::string s = str_url;
        return update_url_user_name_and_password( s, strOptionalUser, strOptionalPassword, bRemoveIfEmpty ) ? s : str_url;
    }
    std::string update_url_user_name_and_password_copy( const std::string & str_url, const std::string & strOptionalUser, const std::string & strOptionalPassword, bool bRemoveIfEmpty ) {
        std::string s = str_url;
        return update_url_user_name_and_password( s, strOptionalUser, strOptionalPassword, bRemoveIfEmpty ) ? s : str_url;
    }

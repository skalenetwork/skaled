...
namespace skutils {
	namespace network {
		...
		void basic_stream_socket_init( int native_fd, std::ostream & osLog ) {
...
#if (defined TCP_KEEPIDLE)
			int ret_keepidle  = ::setsockopt( native_fd, IPPROTO_TCP, TCP_KEEPIDLE, (void *) &timeout, sizeof(int) );
#endif
			int ret_keepintvl = ::setsockopt( native_fd, IPPROTO_TCP, TCP_KEEPINTVL, (void *) &intvl, sizeof(int) );
			int ret_keepinit  = ::setsockopt( native_fd, IPPROTO_TCP, TCP_KEEPCNT, (void *) &probes, sizeof(int) );
			...
			const int ret_rcv_timeoout = setsockopt( native_fd, SOL_SOCKET, SO_RCVTIMEO, &tv_timeout, sizeof(tv_timeout) );
			const int ret_snd_timeoout = setsockopt( native_fd, SOL_SOCKET, SO_SNDTIMEO, &tv_timeout, sizeof(tv_timeout) );
#endif
			...
			if(    ret_keepalive
				...
				osLog << cc::error("Failed to enable keep alive on TCP client socket") << native_fd << cc::error(".");
		}
		...
		std::string get_fd_name_as_url( int fd, const char * strUrlScheme, bool isPeer ) {
			...
			case e_address_family_t::eaft_ip_v6:
				return skutils::tools::format( "%s://%s:%d", ( strUrlScheme != nullptr && strUrlScheme[0] != '\0' ) ? strUrlScheme : "ws", strAddress.c_str(), nPort );
			default:
        ...
        EVP_PKEY * generate_key( size_t nKeyBits ) { // generate a nKeyBits-bit RSA key
			...
			if( ! pkey )
				return nullptr;
			RSA * rsa = RSA_generate_key( nKeyBits, RSA_F4, nullptr, nullptr ); // generate the RSA key and assign it to pkey
		...
		X509 * generate_x509(
			EVP_PKEY * pkey,
			uint64_t ttExpiration,
			const std::map < std::string, std::string > & mapKeyProperties
			...
			X509_gmtime_adj( X509_get_notBefore( x509 ), 0 ); // this certificate is valid from now until exactly one year from now
			X509_gmtime_adj( X509_get_notAfter( x509 ), ttExpiration );
			X509_set_pubkey( x509, pkey ); // set the public key for our certificate
			X509_NAME * name = X509_get_subject_name( x509 ); // we want to copy the subject name to the issuer name
			std::map < std::string, std::string > :: const_iterator itWalk = mapKeyProperties.cbegin(), itEnd = mapKeyProperties.cend();
			for( ; itWalk != itEnd; ++itWalk )
				X509_NAME_add_entry_by_txt( name, itWalk->first.c_str(),  MBSTRING_ASC, (unsigned char *)itWalk->second.c_str(),  -1, -1, 0 ); // set the country code and common name
		...
		bool write_key_to_disk( EVP_PKEY * pkey, const char * strPathToSaveKey ) {
			FILE * f = fopen( ( strPathToSaveKey != nullptr && strPathToSaveKey[0] != '\0' ) ? strPathToSaveKey : "key.pem", "wb" ); // open the PEM file for writing the key to disk
			if( ! f )
				return false;
			bool bSaved = PEM_write_PrivateKey( f, pkey, nullptr, nullptr, 0, nullptr, nullptr ); // write the key to disk.
			fclose( f );
			return bSaved;
		}
		bool write_certifcate_to_disk( X509 * x509, const char * strPathToSaveCertificate ) {
			FILE * f = fopen(  ( strPathToSaveCertificate != nullptr && strPathToSaveCertificate[0] != '\0' ) ? strPathToSaveCertificate : "cert.pem", "wb" ); // open the PEM file for writing the certificate to disk
			if( ! f )
				return false;
        ...
        bool generate_self_signed_certificate_and_key_files(
			const char * strPathToSaveKey,
			const char * strPathToSaveCertificate,
			size_t nKeyBits,
			uint64_t ttExpiration,
			const std::map < std::string, std::string > & mapKeyProperties,
			bool bSkipIfFilesExist,
			...
			if( ! pkey )
				return false;
			X509 * x509 = generate_x509( pkey, ttExpiration, mapKeyProperties ); // generate the x509 certificate
			if( ! x509 ) {
				EVP_PKEY_free( pkey );
				return false;
			}
			bool bSaved = ( write_key_to_disk( pkey, strPathToSaveKey ) && write_certifcate_to_disk( x509, strPathToSaveCertificate ) ) ? true : false; // write the private key and certificate out to disk
			EVP_PKEY_free( pkey );
			X509_free( x509 );
			return bSaved;
		}
		bool generate_self_signed_certificate_and_key_files(
			...
			return
				generate_self_signed_certificate_and_key_files(
					strPathToSaveKey,
...
//#endif
		...
		std::string resolve_address_for_client_connection( int ipVersion, const char * ads, sockaddr46 & sa46 ) {
		std::string cce;
		...
		int n = get_address_info46( ipVersion, ads, (sockaddr46**)&result );
			if( ipVersion == 6 ) {
				...
				case AF_INET:
					...
					::memcpy( &sa46.sa6.sin6_addr.s6_addr[12], &((struct sockaddr_in *)result->ai_addr)->sin_addr, sizeof(struct in_addr) );
					// ...log... ( "uplevelling AF_INET to AF_INET6" );
				break;
				case AF_INET6:
					::memcpy( &sa46.sa6.sin6_addr, &((struct sockaddr_in6 *)result->ai_addr)->sin6_addr, sizeof(struct in6_addr) );
...
//#endif
		...
		bool fd_configure_pipe( int fd ) {
			...
			tv_timeout.tv_sec  = timeout_milli / 1000;
			tv_timeout.tv_usec = timeout_milli % 1000;
			const int ret_rcv_timeoout = ::setsockopt( fd, SOL_SOCKET, SO_RCVTIMEO, &tv_timeout, sizeof(tv_timeout) );
			const int ret_snd_timeoout = ::setsockopt( fd, SOL_SOCKET, SO_SNDTIMEO, &tv_timeout, sizeof(tv_timeout) );
			if(    ret_rcv_timeoout
				|| ret_snd_timeoout
				)
				return false;
		...
		std::list < std::pair < std::string, std::string > > get_machine_ip_addresses( bool is4, bool is6 ) { // first-interface name, second-address
		...
		static std::pair < std::string, std::string > stat_find_ip_addr_non_local( const std::list < std::pair < std::string, std::string > > & lst ) {
			std::pair < std::string, std::string > x;
			std::list < std::pair < std::string, std::string > > :: const_iterator itWalk = lst.cbegin(), itEnd = lst.cend();
			for( ; itWalk != itEnd; ++ itWalk ) {
				const std::string & strIF = itWalk->first;
				if(		strIF == "lo"
					||	strncmp( strIF.c_str(), "virbr", 5 ) == 0
			...
			itWalk = lst.cbegin(); itEnd = lst.cend();
			for( ; itWalk != itEnd; ++ itWalk ) {
				const std::string & strIF = itWalk->first;
				if(		strIF == "lo"
					||	strncmp( strIF.c_str(), "virbr", 5 ) == 0
		...
		std::pair < std::string, std::string > get_machine_ip_address_v4() { // first-interface name, second-address
			std::list < std::pair < std::string, std::string > > lst = get_machine_ip_addresses( true, false ); // first-interface name, second-address
			return stat_find_ip_addr_non_local( lst );
		}
		std::pair < std::string, std::string > get_machine_ip_address_v6() { // first-interface name, second-address
			std::list < std::pair < std::string, std::string > > lst = get_machine_ip_addresses( false, true ); // first-interface name, second-address
			return stat_find_ip_addr_non_local( lst );
		}

...
namespace skutils {
    namespace tools {
        ...
        void replace_all( std::string & s, const std::string & f, const std::string & r ) { // replace (in place)
        ...
        std::string replace_all_copy( const std::string & s, const std::string & f, const std::string & r ) { // replace (copying)
            std::string ret = s;
            replace_all( ret, f, r );
            return ret;
        }
        ...
        std::string to_upper( const std::string & strSrc ) {
            std::string strDst;
            std::transform( strSrc.begin(), strSrc.end(), std::back_inserter < std::string > ( strDst ), ::toupper );
            return strDst;
        }
        std::string to_lower( const std::string & strSrc ) {
            std::string strDst;
            std::transform( strSrc.begin(), strSrc.end(), std::back_inserter < std::string > ( strDst ), ::tolower );
            return strDst;
        }
		...
		std::string get_tmp_file_path( size_t cntCharactersInFileName /*= 16*/ ) { // uses get_tmp_folder_path() and generates tmp file name using generate_random_text_string(cntCharactersInFileName)
			try {
				if( cntCharactersInFileName == 0 )
					return "";
				std::string s = get_tmp_folder_path() + get_path_slash_char() + generate_random_text_string( cntCharactersInFileName );
				return s;
			} catch( ... ) {
				return "";
			}
		...
		bool stat_remove_all_content_in_directory( const char * s ) {
			...
			if( nErrorCode != 0 ) {
				std::cerr << cc::error("Command \"") << cc::p(strCmd) << cc::error("\" returned ") << cc::c(int64_t(nErrorCode)) << cc::error(" error code\n");
				return false;
			}
			return true;
		}
		...
		bool to_bool( const std::string & strValue ) {
			...
			s.erase( s.begin(), std::find_if( s.begin(), s.end(), []( int chr ) { return ! std::isspace( chr ); } ) ); // trim left
			s.erase( std::find_if( s.rbegin(), s.rend(), []( int chr ) { return ! std::isspace( chr ); } ).base(), s.end() ); // trim right
		//  s.erase( s.begin(), std::find_if( s.begin(), s.end(), std::not1( std::ptr_fun<int,int>( std::isspace ) ) ) ); // trim left
		//  s.erase(std::find_if( s.rbegin(), s.rend(), std::not1( std::ptr_fun<int,int>( std::isspace ) ) ).base(), s.end() ); // trim right
			if( s.empty() )
				return false;
		char chr = s[0];
			switch( chr ) {
		...
		std::string nanoseconds_2_lifetime_str( uint64_t ns, bool isColored /*= false*/ ) {
			uint64_t microseconds = ns           / 1000; ns           %= 1000;
			uint64_t milliseconds = microseconds / 1000; microseconds %= 1000;
			uint64_t seconds      = milliseconds / 1000; milliseconds %= 1000;
			uint64_t minutes      = seconds      /   60; seconds      %=   60;
			uint64_t hours        = minutes      /   60; minutes      %=   60;
			uint64_t days         = hours        /   24; hours        %=   24;
			std::string strDays, strHours, strMinutes, strSeconds, strMilliSeconds, strMicroSeconds, strNanoSeconds;
			std::string strDaysSuffix, strHoursSuffix, strMinutesSuffix, strSecondsSuffix, strMilliSecondsSuffix, strMicroSecondsSuffix, strNanoSecondsSuffix;
			std::stringstream ss;
			if( days > 0 ) {
			...
			if( microseconds > 0 || milliseconds > 0 || seconds > 0 || minutes > 0 || hours > 0 || days > 0 ) {
			...
			if( ns > 0 || microseconds > 0 || milliseconds > 0 || seconds > 0 || minutes > 0 || hours > 0 || days > 0 ) {
		...
		float osx_GetSystemMemoryUsagePercentage() {
			...
			if( fpIn ) {
				...
				while( fgets( buf, sizeof(buf), fpIn) != nullptr ) {
					...
					break;  // Stop at "Translation Faults", we don't care about anything at or below that
			}
			pclose( fpIn );
			if( totalPages > 0.0 )
		...
		// you'll need to call this at regular intervals, since it measures the load between the previous call and the current one.
        ...
        std::map<std::string, DiskInfo> disk_load()
        {
            ...
            {
                int i = sscanf(line, "%u %u %s %lu %lu %lu %lu %lu %lu %lu %u %u %u %u",
        ...
        nlohmann::json calculate_load_interval(const std::map<std::string, DiskInfo>& prevLoad, const std::map<std::string, DiskInfo>& currentLoad, size_t nSleepMs)
        {
                 ...
                 ++itPrevLoad, ++itCurrLoad)
            {
                ...
                const double perIter = S_VALUE(prevStat.readIOS + prevStat.writeIOS, currStat.readIOS + currStat.writeIOS, nSleepMs);
        ...
        load_monitor::load_monitor( size_t nSleepMillisecondsBetweenCpuLoadMeasurements /*= 0*/ ) // 0 means use g_nSleepMillisecondsBetweenCpuLoadMeasurements
			: stop_flag_( false )
			, cpu_load_( 0.0 )
			, nSleepMillisecondsBetweenCpuLoadMeasurements_( nSleepMillisecondsBetweenCpuLoadMeasurements )
		{
        ...
        void load_monitor::calculate_load( size_t nSleep )
        {
            ...
            cpu_load_ = ((cpuCurrentLoad[0]+cpuCurrentLoad[1]+cpuCurrentLoad[2]) - (cpuPrevLoad[0]+cpuPrevLoad[1]+cpuPrevLoad[2])) /
                    ((cpuCurrentLoad[0]+cpuCurrentLoad[1]+cpuCurrentLoad[2]+cpuCurrentLoad[3]) - (cpuPrevLoad[0]+cpuPrevLoad[1]+cpuPrevLoad[2]+cpuPrevLoad[3]));
        ...
        double mem_usage() { // 0.0...1.0
            ...
            double lfPercentAvail = double( lfTotalPages - lfFreePages ) / double( lfTotalPages ); // 0.0...1.0
		...
		std::string create_random_uuid( const char * strSeparator ) {
			...
			if( strSeparator == nullptr )
				strSeparator = "";
			::sprintf(
				strUuid, "%x%x%s%x%s%x%s%x%s%x%x%x",
				std::rand(), std::rand(), strSeparator,          // generates a 64-bit Hex number
				std::rand(), strSeparator,                       // generates a 32-bit Hex number
				((std::rand() & 0x0fff) | 0x4000), strSeparator, // generates a 32-bit Hex number of the form 4xxx (4 indicates the UUID version)
				std::rand() % 0x3fff + 0x8000, strSeparator,     // generates a 32-bit Hex number in the range [0x8000, 0xbfff]
...
//	#endif
		...
		void md5::update( FILE * f ){
			// MD5 update for files. Like above, except that it works on files (and uses above as a primitive.)
			if( ! f )
				return;
		...
		unsigned char * md5::raw_digest() {
			...
			if( ! finalized ) {
				//std::cerr << "md5::raw_digest: cannot get digest if you haven't finalized the digest" << std::endl;
		...
		char * md5::hex_digest() {
			...
			::memset( s, 0, 33 );
			if( ! finalized ){
				//std::cerr << "md5::hex_digest: cannot get digest if you haven't finalized the digest!" << std::endl;
				return s;
			}
			for( i = 0; i < 16; i++ )
		...
		// Constants for MD5Transform routine. Although we could use C++ style constants, defines are actually better, since they let us easily evade scope clashes.
		#define S11 7
		#define S12 12
		#define S13 17
		#define S14 22
		...
		inline unsigned int md5::rotate_left ( uint4 x, uint4 n ) { return (x << n) | (x >> (32-n)); } // ROTATE_LEFT rotates x left n bits.
		inline unsigned int md5::F( uint4 x, uint4 y, uint4 z ) { return (x & y) | (~x & z); } // F, G, H and I are basic MD5 functions.
		...
		inline void md5::FF( uint4 & a, uint4 b, uint4 c, uint4 d, uint4 x, uint4 s, uint4 ac ) { a += F( b, c, d ) + x + ac; a = rotate_left( a, s ) + b; }
		inline void md5::GG( uint4 & a, uint4 b, uint4 c, uint4 d, uint4 x, uint4 s, uint4 ac ) { a += G( b, c, d ) + x + ac; a = rotate_left( a, s ) + b; }
		inline void md5::HH( uint4 & a, uint4 b, uint4 c, uint4 d, uint4 x, uint4 s, uint4 ac ) { a += H( b, c, d ) + x + ac; a = rotate_left( a, s ) + b; }
		inline void md5::II( uint4 & a, uint4 b, uint4 c, uint4 d, uint4 x, uint4 s, uint4 ac ) { a += I( b, c, d ) + x + ac; a = rotate_left( a, s ) + b; }
	...
	namespace terminal {
		...
		char getch_no_wait() {
			...
			char buf = 0;
			struct termios old; ::memset( &old, 0, sizeof(struct termios) );
	...
	namespace signal {
		...
		bool get_signal_description( int nSignalNo, std::string & strSignalName, std::string & strSignalDescription ) { // returns true if signal name is known
			struct sig_desc_t {
				int nSignalNo;
			...
			static const sig_desc_t g_arr[] = {
				{ SIGINT, "SIGINT", "This signal is the same as pressing ctrl-c. On some systems, \"delete\" + \"break\" sends the same signal to the process. The process is interrupted and stopped. However, the process can ignore this signal." },
				{ SIGTERM, "SIGTERM", "Requests a process to stop running. This signal can be ignored. The process is given time to gracefully shutdown. When a program gracefully shuts down, that means it is given time to save its progress and release resources. In other words, it is not forced to stop. SIGINT is very similar to SIGTERM." },
				{ SIGKILL, "SIGKILL", "Forces the process to stop executing immediately. The program cannot ignore this signal. This process does not get to clean-up either." },
				{ SIGHUP, "SIGHUP", "Disconnects a process from the parent process. This an also be used to restart processes. For example, \"killall -SIGUP compiz\" will restart Compiz. This is useful for daemons with memory leaks." },
				{ SIGQUIT, "SIGQUIT", "The SIGQUIT sigma; is like SIGINT with the ability to make the process produce a core dump." },
				{ SIGILL, "SIGILL", "When a process performs a faulty, forbidden, or unknown function, the system sends the SIGILL signal to the process. This is the ILLegal SIGnal." },
				{ SIGABRT, "SIGABRT", "This is the abort signal. Typically, a process will initiate this kill signal on itself." },
				{ SIGSTOP, "SIGSTOP", "This signal makes the operating system pause a process's execution. The process cannot ignore the signal." },
				{ SIGFPE, "SIGFPE", "Processes that divide by zero are killed using SIGFPE. Imagine if humans got the death penalty for such math. NOTE: The author of this article was recently drug out to the street and shot for dividing by zero." },
				{ SIGSEGV, "SIGSEGV", "When an application has a segmentation violation, this signal is sent to the process." },
				{ SIGTSTP, "SIGTSTP", "This signal is like pressing ctrl-z. This makes a request to the terminal containing the process to ask the process to stop temporarily. The process can ignore the request." },
				{ SIGCONT, "SIGCONT", "To make processes continue executing after being paused by the SIGTSTP or SIGSTOP signal, send the SIGCONT signal to the paused process. This is the CONTinue SIGnal. This signal is beneficial to Unix job control (executing background tasks)." },
				{ SIGALRM, "SIGALRM", "Sent when the real time or clock time timer expires." },
				{ SIGTRAP, "SIGTRAP", "This signal is used for debugging purposes. When a process has performed an action or a condition is met that a debugger is waiting for, this signal will be sent to the process." },
				{ SIGBUS, "SIGBUS", "When a process is sent the SIGBUS signal, it is because the process caused a bus error. Commonly, these bus errors are due to a process trying to use fake physical addresses or the process has its memory alignment set incorrectly." },
				{ SIGUSR1, "SIGUSR1", "This indicates a user-defined condition. This signal can be set by the user by programming the commands in sigusr1.c. This requires the programmer to know C/C++." },
				{ SIGUSR2, "SIGUSR2", "This indicates a user-defined condition." },
				{ SIGPIPE, "SIGPIPE", "When a process tries to write to a pipe that lacks an end connected to a reader, this signal is sent to the process. A reader is a process that reads data at the end of a pipe." },
				{ SIGCHLD, "SIGCHLD", "When a parent process loses its child process, the parent process is sent the SIGCHLD signal. This cleans up resources used by the child process. In computers, a child process is a process started by another process know as a parent." },
				{ SIGTTIN, "SIGTTIN", "When a process attempts to read from a tty (computer terminal), the process receives this signal." },
				{ SIGTTOU, "SIGTTOU", "When a process attempts to write from a tty (computer terminal), the process receives this signal." },
				{ SIGURG, "SIGURG", "When a process has urgent data to be read or the data is very large, the SIGURG signal is sent to the process." },
				{ SIGXCPU, "SIGXCPU", "When a process uses the CPU past the allotted time, the system sends the process this signal. SIGXCPU acts like a warning; the process has time to save the progress (if possible) and close before the system kills the process with SIGKILL." },
				{ SIGXFSZ, "SIGXFSZ", "Filesystems have a limit to how large a file can be made. When a program tries to violate this limit, the system will send that process the SIGXFSZ signal." },
				{ SIGVTALRM, "SIGVTALRM", "Sent when CPU time used by the process elapses." },
				{ SIGPROF, "SIGPROF", "Sent when CPU time used by the process and by the system on behalf of the process elapses." },
				{ SIGWINCH, "SIGWINCH", "When a process is in a terminal that changes its size, the process receives this signal." },
				{ SIGIO, "SIGIO", "Alias to SIGPOLL or at least behaves much like SIGPOLL." },
#if (defined SIGPWR)
				{ SIGPWR, "SIGPWR", "Power failures will cause the system to send this signal to processes (if the system is still on)." },
#endif
				{ SIGSYS, "SIGSYS", "Processes that give a system call an invalid parameter will receive this signal." },
#if (defined SIGEMT)
				{ SIGEMT, "SIGEMT", "Processes receive this signal when an emulator trap occurs." },
#endif
#if (defined SIGINFO)
				{ SIGINFO, "SIGINFO", "Terminals may sometimes send status requests to processes. When this happens, processes will also receive this signal." },
#endif
#if (defined SIGLOST)
			...
			if( nRangeMin <= nSignalNo && nSignalNo <= nRangeMax ) {
				...
				std::stringstream ss;
				ss << "Signal " << nSignalNo << " is in range [SIGRTMIN(" << nRangeMin << ")...SIGRTMAX(" << nRangeMax <<  ")]."
				   << " Distance from SIGRTMIN is " << nDistFromMin << "."
				   << "This is a set of signals that varies between systems."
				   << " They are labeled SIGRTMIN+1, SIGRTMIN+2, SIGRTMIN+3, ......., and so on (usually up to 15)."
				   << " These are user-defined signals; they must be programmed in the Linux kernel's source code."
		...
		std::string signal2str( int nSignalNo, const char * strPrefix, const char * strSuffix, bool isAddDesc ) {
		...
		bool init_common_signal_handling( fn_signal_handler_t fnSignalHander ) {
			...
			//
			// Handled:
			// The SIGINT signal is the same as pressing ctrl-c. On some systems, "delete" + "break" sends the same signal to the process. The process is interrupted and stopped. However, the process can ignore this signal.
			...
			// The SIGHUP signal disconnects a process from the parent process. This an also be used to restart processes. For example, "killall -SIGUP compiz" will restart Compiz. This is useful for daemons with memory leaks.
			...
			// The SIGABRT signal is the abort signal. Typically, a process will initiate this kill signal on itself.
		...
		void print_backtrace() {
			...
			int cntBacktrace = ::backtrace( arrBacktrace, sizeof(arrBacktrace)/sizeof(arrBacktrace[0]) ); // get void*'s for all entries on the stack

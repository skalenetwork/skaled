...
//static inline void LOCAL_DEBUG_TRACE( const std::string & x ) { if( x.empty() ) return; std::cout.flush(); std::cerr.flush(); std::cout << x << "\n"; std::cout.flush(); }

namespace skutils {
	namespace dispatch {

		skutils::multithreading::recursive_mutex_type & get_dispatch_mtx() {
			//static skutils::multithreading::recursive_mutex_type g_dispatch_mtx( "skutils::dispatch::g_dispatch_mtx" );
		...
		domain_ptr_t default_domain(
			const size_t nNumberOfThreads, // = 0 // 0 means use CPU count
		...
		queue_ptr_t get_auto_removeable() {
			...
			if( ! pQueue )
				throw std::runtime_error("dispatch simple get_auto_removeable() API failed, bad auto queue " + id);
		...
		priority_t priority( const queue_id_t & id ) { // get
			queue_ptr_t pQueue( get( id ) );
			if( ! pQueue )
				throw std::runtime_error("dispatch simple priority() API (getter) failed, bad queue " + id);
			return pQueue->priority();
		}
		void priority( const queue_id_t & id, priority_t pri ) { // set
			queue_ptr_t pQueue( get( id ) );
			if( ! pQueue )
				throw std::runtime_error("dispatch simple priority() API (setter) failed, bad queue " + id);
			pQueue->priority( pri );
		}

		void async( const queue_id_t & id, job_t /*&*/ fn, duration_t timeout /*= duration_t(0)*/, duration_t interval /*= duration_t(0)*/, job_id_t * pJobID /*= nullptr*/ ) {
		...
		void async( job_t /*&*/ fn, duration_t timeout /*= duration_t(0)*/, duration_t interval /*= duration_t(0)*/, job_id_t * pJobID /*= nullptr*/ ) {
		...
		void once( const queue_id_t & id, job_t /*&*/ fn, duration_t timeout, job_id_t * pJobID /*= nullptr*/ ) {
			queue_ptr_t pQueue( id.empty() ? get_auto_removeable() : get( id ) );
			if( ! pQueue )
		...
		void repeat( const queue_id_t & id, job_t /*&*/ fn, duration_t interval, job_id_t * pJobID /*= nullptr*/ ) {
			queue_ptr_t pQueue( id.empty() ? get_auto_removeable() : get( id ) );
			if( ! pQueue )
		...
		loop::mutex_type & loop::loop_mtx() const { return loop_mtx_; }
		...
		bool loop::wait_until_startup(
			duration_t timeout, // = duration_t(0) // zero means no timeout
			duration_t step // = duration_t(10*1000*1000)
			) {
			return
				sleep_while_true(
					[ this ] () -> bool { return (!is_running()); },
					timeout,
					step
					);
		}
		...
		loop::job_data_t::job_data_t( loop_ptr_t pLoop )
			: pLoop_( pLoop )
			, needsCancel_( false )
		{
		...
		void loop::job_data_t::init() {
			...
			// milliseconds
			uint64_t ms_timeout  = ( ns_timeout  / (1000*1000) ) + ( ( ( ns_timeout  % (1000*1000) ) != 0 ) ? 1 : 0 );
			uint64_t ms_interval = ( ns_interval / (1000*1000) ) + ( ( ( ns_interval % (1000*1000) ) != 0 ) ? 1 : 0 );
			uv_timer_start(
				p_uvTimer,
				[] ( uv_timer_t * pTimer ) {
		...
		void loop::impl_job_add( const job_id_t & id, job_t /*&*/ fn, duration_t timeout, duration_t interval ) {
			if( impl_job_present( id ) )
				throw std::runtime_error( "loop need unique job identifier, but specified \"" + id + "\" is not unique" );
			if( ! fn )
		...
		void loop::job_add_once( const job_id_t & id, job_t /*&*/ fn, duration_t timeout ) { // like JavaScript setTimeout()
			job_add( id, fn, timeout, duration_t(0) );
		}
		void loop::job_add_periodic( const job_id_t & id, job_t /*&*/ fn, duration_t interval ) { // like JavaScript setInterval()
		...
		queue::mutex_type & queue::mtx_jobs() const { return mtx_jobs_; }
		queue::mutex_type & queue::mtx_run() const { return mtx_run_; }
		domain_ptr_t queue::get_domain() { return pDomain_; }
		bool queue::is_detached() {
			if( ! pDomain_ )
				return true;
			return false;
		}
		queue_id_t queue::get_id() const { return id_; }
		...
		bool queue::impl_job_add( job_t /*&*/ fn, duration_t timeout, duration_t interval, job_id_t * pJobID ) { // if both timeout and interval are zero, then invoke once asyncronously
			try {
				if( pJobID )
					pJobID->clear();
				if( ! fn )
		...
		bool queue::job_add(
			job_t /*&*/ fn,
			duration_t timeout, // = duration_t(0) // if both timeout and interval are zero, then invoke once asyncronously
		...
		//	, mtx_with_jobs_( skutils::tools::format("skutils::dispatch::domain-%p/mutex/with_jobs", this ) )
			, shutdown_flag_( true )
			, thread_pool_( ( nNumberOfThreads > 0 ) ? nNumberOfThreads : skutils::tools::cpu_count(), nQueueLimit )
		...
		domain::mutex_type & domain::domain_mtx()    const { return get_dispatch_mtx(); }
		domain::mutex_type & domain::mtx_with_jobs() const { return domain_mtx();       }
		...
		queue_ptr_t domain::impl_queue_get(
			const queue_id_t & id,
		...
		bool domain::impl_queue_remove( const queue_id_t & id ) {
			{ // block
				...
				sleep_while_true( [&] () -> bool { return pQueue->is_running(); } ); // wait for async tasks to complete, if any
			...
			{ // block
				...
				queues_with_jobs_set_t::iterator itFound =
					std::find_if( itFrom, itTo,
					[&id] ( const queue_ptr_t & pQueue ) -> bool {
							if( pQueue->get_id() == id )
								return true;
							return false;
						} );
				if( itFound != itTo ) {
					...
					LOCAL_DEBUG_TRACE( cc::sunny("domain::impl_queue_remove()") + cc::debug(" did ") + cc::error("erased") + cc::debug(" already-removed queue ") + cc::bright(id) + cc::debug(", ") + cc::sunny("with_jobs_.size()") + cc::debug("=") + cc::size10(with_jobs_.size()) );
				}
			} // block
			return true;
		}
...
//				return;
		...
		queue_ptr_t domain::impl_find_queue_to_run() { // find queue with minimal accumulator and remove it from with_jobs_
			...
			for( ; itWalk != itEnd; ) {
				...
				if( pQueue->is_removed() ) {
					LOCAL_DEBUG_TRACE( cc::sunny("domain::impl_find_queue_to_run()") + cc::debug(" will ") + cc::success("erase") + cc::debug(" pre-removed queue ") + cc::bright(pQueue->get_id()) + cc::debug(", ") + cc::sunny("with_jobs_.size()") + cc::debug("=") + cc::size10(with_jobs_.size()) );
					itWalk = with_jobs_.erase( itWalk );
					itEnd = with_jobs_.end();
					continue;
				}
				if( pQueue->async_job_count() == 0 ) {
					LOCAL_DEBUG_TRACE( cc::sunny("domain::impl_find_queue_to_run()") + cc::debug(" will ") + cc::success("erase") + cc::debug(" no-jobs queue ") + cc::bright(pQueue->get_id()) + cc::debug(", ") + cc::sunny("with_jobs_.size()") + cc::debug("=") + cc::size10(with_jobs_.size()) );
					itWalk = with_jobs_.erase( itWalk );
					itEnd = with_jobs_.end();
					continue;
				}
				if( pQueue->awaiting_sync_run_ > 0 ) {
					//++ itWalk;
					LOCAL_DEBUG_TRACE( cc::sunny("domain::impl_find_queue_to_run()") + cc::debug(" will ") + cc::success("erase") + cc::debug(" awaiting sync-run queue ") + cc::bright(pQueue->get_id()) + cc::debug(", ") + cc::sunny("with_jobs_.size()") + cc::debug("=") + cc::size10(with_jobs_.size()) );
					itWalk = with_jobs_.erase( itWalk );
					itEnd = with_jobs_.end();
					continue;
				}
				...
				LOCAL_DEBUG_TRACE( cc::sunny("domain::impl_find_queue_to_run()") + cc::debug(" did ") + cc::success("successfully") + cc::debug(" fetched queue ") + cc::bright(pQueueFound->get_id()) + cc::debug(", ") + cc::sunny("with_jobs_.size()") + cc::debug("=") + cc::size10(with_jobs_.size()) );
				break;
			} // for( ; itWalk != itEnd; )
			return pQueueFound;
		}
		void domain::impl_decrease_accumulators( priority_t acc_subtract ) {
			...
			uint64_t counter = uint64_t(decrease_accumulators_counter_), period = uint64_t(decrease_accumulators_period_);
			if( period > 1 ) {
...
//				};
//			pLoop->on_job_exception_ = [&] ( const skutils::dispatch::job_id_t & id, std::exception * pe ) -> void {
//				};
			loop_thread_ = std::thread( [ pLoop ] () -> void {
					pLoop->run();
				} );
...
//				} );
		...
		void domain::on_queue_job_added( queue & q ) {
			{ // block
				...
				if( cntJobs > 0 && (!q.is_running()) ) {
					with_jobs_.insert( &q );
					LOCAL_DEBUG_TRACE( cc::sunny("domain::on_queue_job_added()") + cc::debug(" did added work-able queue ") + cc::bright(q.get_id()) + cc::debug(", ") + cc::sunny("with_jobs_.size()") + cc::debug("=") + cc::size10(with_jobs_.size()) );
				}
			} // block
			//fetch_lock_.notify_all();
			fetch_lock_.notify_one();
		...
		void domain::on_queue_job_complete( queue & q ) {
			{ // block
				...
				if( cntJobs > 0 && (!q.is_running()) ) {
					with_jobs_.insert( &q );
					LOCAL_DEBUG_TRACE( cc::sunny("domain::on_queue_job_complete()") + cc::debug(" did added work-able queue ") + cc::bright(q.get_id()) + cc::debug(", ") + cc::sunny("with_jobs_.size()") + cc::debug("=") + cc::size10(with_jobs_.size()) );
				}
			} // block
			//fetch_lock_.notify_all();
			fetch_lock_.notify_one();

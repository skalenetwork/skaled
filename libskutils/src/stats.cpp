...
namespace skutils {
    namespace stats {
        ...
        event_record_item_t::event_record_item_t( time_point a_time_stamp_ ) : time_stamp_( a_time_stamp_ ) { }
        event_record_item_t::event_record_item_t( event_record_item_t::myrct x ) { assign( x ); }
        ...
        int event_record_item_t::compare( event_record_item_t::myrct x ) const {
            if( time_stamp_ < x.time_stamp_ ) return -1;
            if( time_stamp_ > x.time_stamp_ ) return  1;
            return 0;
        }
        ...
        void named_event_stats::event_queue_add( const std::string & strQueueName, size_t nSize ) {
            ...
            if (itEvent == std::end(m_Events))
            {
                m_Events.emplace(std::piecewise_construct,
                                 std::forward_as_tuple(strQueueName),
        ...
        double named_event_stats::compute_eps_from_start(const std::string& eventName, const time_point& tpNow) const
        {
            auto itEvent = m_Events.find(eventName);
            if (itEvent == std::end(m_Events))
                return 0.0;

            const auto duration_milliseconds = std::chrono::duration_cast < std::chrono::milliseconds > (tpNow - itEvent->second.m_startTime);
        ...
        double named_event_stats::compute_eps(const std::string& eventName, const time_point& tpNow, size_t * p_nSummary) const
        {
            auto itEvent = m_Events.find(eventName);
            if (itEvent == std::end(m_Events))
                return 0.0;
        ...
        double named_event_stats::compute_eps(t_NamedEventsIt& itEvent, const time_point& tpNow, size_t * p_nSummary) const
        {
            const auto duration_milliseconds = std::chrono::duration_cast < std::chrono::milliseconds > (tpNow - itEvent->second.m_prevTime);
            const double seconds = static_cast<double>(duration_milliseconds.count()) / 1000.0;
        ...
        std::string named_event_stats::getEventStatsDescription( time_point tpNow, bool isColored /*= false*/, bool bSkipEmptyStats /*= true*/, bool bWithSummaryAsSuffix /*= false*/ ) const
        {
        ...
        std::string named_event_stats::getEventStatsDescription( bool isColored /*= false*/, bool bSkipEmptyStats /*= true*/, bool bWithSummaryAsSuffix /*= false*/ ) const {
            return getEventStatsDescription( clock::now(), isColored, bSkipEmptyStats, bWithSummaryAsSuffix );
        }

        nlohmann::json named_event_stats::toJSON( time_point tpNow, bool bSkipEmptyStats /*= true*/, const std::string& prefix ) const {
            ...
            if (!m_Events.empty())
            {
                ...
                size_t i = 0;
                for (auto itCurrent = std::begin(m_Events); itCurrent != std::end(m_Events); ++itCurrent)
                {
        ...
        traffic_record_item_t::traffic_record_item_t( time_point a_time_stamp_, bytes_count_t a_bytes_ ) : time_stamp_( a_time_stamp_ ), bytes_( a_bytes_ ) { }
        traffic_record_item_t::traffic_record_item_t( bytes_count_t a_bytes_ ) : time_stamp_( clock::now() ), bytes_( a_bytes_ ) { }
        traffic_record_item_t::traffic_record_item_t( traffic_record_item_t::myrct x ) { assign( x ); }
    ...
    {
        double stat_compute_bps( const traffic_queue_t & qtr, time_point tpNow, bytes_count_t * p_nSummary /*= nullptr*/ )
        {
            if( p_nSummary )
            ...
            const auto duration_milliseconds = std::chrono::duration_cast < std::chrono::milliseconds > ( /*to.time_stamp_*/ /*clock::now()*/ tpNow - from.time_stamp_ );
        ...
        double stat_compute_bps_last_known( const traffic_queue_t & qtr, bytes_count_t * p_nSummary /*= nullptr*/ )
        {
            if( p_nSummary )
                (*p_nSummary) = 0;
            if (qtr.empty())

...
namespace skutils {
	namespace mail {
		...
		smtp_x_priority str_2_smtp_x_priority( const std::string & s ) {
			...
			char c0 = x[0];
			switch( c0 ) {
			case 'h': return smtp_x_priority::XPRIORITY_HIGH;
			case 'l': return smtp_x_priority::XPRIORITY_LOW;
			} // switch( c0 )
			return smtp_x_priority::XPRIORITY_NORMAL;
		}
		std::string smtp_x_priority_2_str( smtp_x_priority sxp ) {
			switch( sxp ) {
			case smtp_x_priority::XPRIORITY_HIGH: return "high";
			case smtp_x_priority::XPRIORITY_LOW:  return "low";
		...
		smtp_security_type str_2_smtp_security_type( const std::string & s ) {
			...
			case 'z':
			case '0':
			case 'n': return smtp_security_type::NO_SECURITY;
			case 't': return smtp_security_type::USE_TLS;
			case 's': return smtp_security_type::USE_SSL;
			} // switch( c0 )
			return smtp_security_type::DO_NOT_SET;
		}
		std::string smtp_security_type_2_str( smtp_security_type sst ) {
			switch( sst ) {
			case smtp_security_type::NO_SECURITY: return "none";
			case smtp_security_type::USE_TLS:     return "tls";
			case smtp_security_type::USE_SSL:     return "ssl";
		...
		command_entry command_list[] = {
			...
			{ command_DATABLOCK,     3*60,  0,     0,   smtp_error_info::COMMAND_DATABLOCK },	// Here the valid_reply_code is set to zero because there are no replies when sending data blocks
		...
		void client::send() {
			...
			if( socket_handle_ == INVALID_SOCKET ) {
				if( ! connectRemoteServer( strSMTPSrvName_.c_str(), nSMTPServerPort_, security_type_, bAuthenticate_ ) )
			...
			try {
				...
				for( FileId = 0; FileId < vecAttachments_.size(); FileId++ ) {
					...
					EncodedFileName += skutils::tools::base64::encode((unsigned char *) FileName.c_str(), FileName.size() );
					EncodedFileName += "?=";
					...
					for( i = 0; i < FileSize/54+1; i++ ) {
						...
						MsgPart ? strcat( pSendBuf_, skutils::tools::base64::encode(reinterpret_cast<const unsigned char*>( FileBuf),res).c_str() )
								: strcpy( pSendBuf_, skutils::tools::base64::encode(reinterpret_cast<const unsigned char*>( FileBuf),res).c_str() );
						strcat( pSendBuf_,"\r\n" );
						MsgPart += res + 2;
		...
		bool client::connectRemoteServer(
			...
			try {
				...
				if( connect( socket_handle_,(LPSOCKADDR)&sockAddr, sizeof(sockAddr) ) == SOCKET_ERROR ) {
#if (defined WIN32)
				...
				while( true ) {
					...
					if((res = select( socket_handle_+1,nullptr,&fdwrite,&fdexcept,&timeout)) == SOCKET_ERROR)
					{
#if (defined WIN32)
						closesocket( socket_handle_ );
#else
				...
				if( b_authenticate && isKeywordSupported( pRecvBuf_, "AUTH" ) == true ) {
					...
					if( isKeywordSupported( pRecvBuf_, "LOGIN" ) == true ) {
						...
						// send login:
						std::string encoded_login = skutils::tools::base64::encode(reinterpret_cast<const unsigned char*>(strLogin_.c_str()),strLogin_.size() );
						...
						// send password:
						std::string encoded_password = skutils::tools::base64::encode(reinterpret_cast<const unsigned char*>(strPassword_.c_str()),strPassword_.size() );
						...
						snprintf( pSendBuf_, BUFFER_SIZE, "%s^%s^%s", strLogin_.c_str(), strLogin_.c_str(), strPassword_.c_str() );
						...
						encoded_challenge = skutils::tools::base64::encode(reinterpret_cast<const unsigned char*>(decoded_challenge.c_str()),decoded_challenge.size() );
						...
						if( strstr( pRecvBuf_, "charset") )
							snprintf( pSendBuf_, BUFFER_SIZE, "charset=utf-8,username=\"%s\"", strLogin_.c_str() );
						else
						...
						encoded_challenge = skutils::tools::base64::encode( ustrDigest, strlen( pSendBuf_ ) );
						delete[] ustrDigest;
						snprintf( pSendBuf_, BUFFER_SIZE, "%s\r\n", encoded_challenge.c_str() );
						pEntry = findCommandEntry( command_DIGESTMD5 );
						sendData( pEntry );
					...
					else throw smtp_error_info( smtp_error_info::LOGIN_NOT_SUPPORTED );
				}
			} catch( const smtp_error_info & ) {
		...
		void client::formatHeader( char * header ) {
			...
			strcat( header,"From: " );
			if( strNameFrom_.size() ) strcat( header, strNameFrom_.c_str() );
			...
			if( bReadReceipt_ ) {
				...
				if( strReplyTo_.size() ) strcat( header, strReplyTo_.c_str() );
				else strcat( header, strNameFrom_.c_str() );
				strcat( header, "\r\n" );
			}

			// X-Priority: <SP> <number> <CRLF>
			switch( eXPriority_ ) {
			case XPRIORITY_HIGH: strcat( header,"X-Priority: 2 (High)\r\n" ); break;
			case XPRIORITY_NORMAL: strcat( header,"X-Priority: 3 (Normal)\r\n" ); break;
			case XPRIORITY_LOW: strcat( header,"X-Priority: 4 (Low)\r\n" ); break;
			default: strcat( header,"X-Priority: 3 (Normal)\r\n" ); break;
			}
			...
			if( ! vecAttachments_.size() ) { // no attachments
				if( bIsHTML_) strcat( header, "Content-Type: text/html; charset=\"" );
			...
			{ // there is one or more attachments
				...
				strcat( pSendBuf_,"\r\n" );
				if( bIsHTML_) strcat( pSendBuf_,"Content-type: text/html; charset=" );
				else strcat( pSendBuf_,"Content-type: text/plain; charset=" );
				strcat( header, strCharSet_.c_str() );
				strcat( header, "\r\n" );
		...
		void client::receiveData( command_entry * pEntry ) {
			...
			if( ( res = select( socket_handle_+1, &fdread, nullptr, nullptr, &time)) == SOCKET_ERROR ) {
		...
		void client::sendData( command_entry * pEntry ) {
			...
			while( nLeft > 0 ) {
				FD_ZERO( &fdwrite );
				FD_SET( socket_handle_, &fdwrite );
				if( ( res = select( socket_handle_+1, nullptr, &fdwrite, nullptr, &time ) ) == SOCKET_ERROR ) {
					FD_CLR( socket_handle_,&fdwrite );
					throw smtp_error_info( smtp_error_info::WSA_SELECT );
				}
		...
		void client::setSMTPServer( const char *  SrvName, const unsigned short SrvPort, bool b_authenticate ) {
		...
		void client::receiveData_SSL( SSL * ssl, command_entry * pEntry ) {
			...
			while( ! bFinish ) {
				...
				if( ( res = select( socket_handle_+1, &fdread, &fdwrite, nullptr, &time ) ) == SOCKET_ERROR ) {
					FD_ZERO( &fdread );
					FD_ZERO( &fdwrite );
					throw smtp_error_info( smtp_error_info::WSA_SELECT );
				}
				...
				if( FD_ISSET( socket_handle_, &fdread ) || ( read_blocked_on_write && FD_ISSET( socket_handle_, &fdwrite ) ) ) {
					while( 1 ) {
		...
		void client::sendData_SSL( SSL * ssl, command_entry * pEntry ) {
			...
			while( nLeft > 0 ) {
				...
				if( FD_ISSET( socket_handle_, &fdwrite ) || ( write_blocked_on_read && FD_ISSET( socket_handle_, &fdread ) ) ) {

...
client::client( const skutils::url& u,
    ...
    : u_( u ), timeout_milliseconds_( timeout_milliseconds ) {
    if ( pOptsSSL )
        optsSSL = ( *pOptsSSL );
}
...
bool client::is_valid() const {
    ...
    const std::string strScheme =
        skutils::tools::to_lower( skutils::tools::trim_copy( u_.scheme() ) );
...
bool client::query( const char* strInData,
    ...
    try {
        ...
        std::string strHeaderContentType = skutils::tools::format(
            "Content-Type: %s", strInContentType ? strInContentType : "application/json" );
        ...
        if ( is_ssl_with_explicit_cert_key() ) {
            if ( pCurlCryptoEngine_ ) {
                ...
                // set the crypto engine as default, only needed for the first time you load a
                // engine in a curl object
            ...
            curl_easy_setopt( curl, CURLOPT_SSLCERT, optsSSL.client_cert.c_str() );  // like
                                                                                     // "cert.pem"
        ...
        if ( curl_code != CURLE_OK )
            throw std::runtime_error(
                std::string( "CURL easy perform failed: " ) + curl_easy_strerror( curl_code ) );
        long http_code;
        curl_easy_getinfo( curl, CURLINFO_RESPONSE_CODE, &http_code );
        if ( !( http_code == 200 && curl_code != CURLE_ABORTED_BY_CALLBACK ) ) {
            throw std::runtime_error(
                skutils::tools::format( "CURL failed with code %d=0x%X, HTTP status is  %d=0x%X",
                    int( curl_code ), int( curl_code ), int( http_code ), int( http_code ) ) );
        }
        char* ct = nullptr;

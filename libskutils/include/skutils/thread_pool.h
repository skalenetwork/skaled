...
namespace skutils {
    ...
    public:
		thread_safe_queue( const size_t nQueueLimit = 0 ) : mutex_( "MTX-THREAD-SAFE-QUEUE" ), limit_( nQueueLimit ) { }
		...
		thread_safe_queue & limit( size_t nQueueLimit ) { limit_ = nQueueLimit; return (*this); }
    ...
    private:
		...
		std::condition_variable_any conditional_lock_; // std::condition_variable_any conditional_lock_;
        void init();
        void shutdown();
    public:
		thread_pool(
			const size_t nNumberOfThreads,
		...
		thread_pool & queue_limit( size_t n ) { queue_.limit( n ); return (*this); }

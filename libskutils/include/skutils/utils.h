...
//        std::string format_no_throw( const char * format, va_list args );
		...
		template < typename ... Args > inline std::string format( const char * format, Args ... args ) {
		...
		template < typename ... Args > inline std::string format_no_throw( const char * format, Args ... args ) {
			try {
				char * null_str = nullptr;
				size_t size = std::snprintf( null_str, 0, format, args ... ) + 1; // extra space for '\0'
				std::unique_ptr < char[] > buf( new char[ size ] );
		...
		inline bool default_trim_what( unsigned char ch ) { return std::isspace( ch ) || ch == '\r' || ch == '\n' || ch == '\t'; }
        inline void ltrim( std::string & s, std::function<bool(unsigned char)> f = default_trim_what ) { // trim from start (in place)
            s.erase(
                s.begin(),
                std::find_if( s.begin(), s.end(), [&] ( unsigned char ch ) { return ! f( ch ); } )
                );
        }
        inline void rtrim( std::string & s, std::function<bool(unsigned char)> f = default_trim_what ) { // trim from end (in place)
            s.erase(
               std::find_if( s.rbegin(), s.rend(), [&] ( unsigned char ch ) { return ! f( ch ); } ).base(),
               s.end()
               );
        }
        inline void trim( std::string & s, std::function<bool(unsigned char)> f = default_trim_what ) { ltrim( s, f ); rtrim( s, f ); } // trim from both ends (in place)
        inline std::string ltrim_copy( std::string s, std::function<bool(unsigned char)> f = default_trim_what ) { ltrim( s, f ); return s; }  // trim from start (copying)
        inline std::string rtrim_copy( std::string s, std::function<bool(unsigned char)> f = default_trim_what ) { rtrim( s, f ); return s; } // trim from end (copying)
        inline std::string trim_copy( std::string s, std::function<bool(unsigned char)> f = default_trim_what ) { trim( s, f ); return s; } // trim from both ends (copying)
        ...
        extern void replace_all( std::string & s, const std::string & f, const std::string & r ); // replace (in place)
        extern std::string replace_all_copy( const std::string & s, const std::string & f, const std::string & r ); // replace (copying)
		...
		extern std::string get_tmp_file_path( size_t cntCharactersInFileName = 16 ); // uses get_tmp_folder_path() and generates tmp file name using generate_random_text_string(cntCharactersInFileName)
		...
		// json string escapement is based on https://github.com/ATNoG/JSONObject/blob/nativeCpp/StringUtils.cpp
        ...
        extern nlohmann::json calculate_load_interval(const std::map<std::string, DiskInfo>& prevLoad, const std::map<std::string, DiskInfo>& currentLoad, size_t nSleepMs);
#endif
...
#endif
		public:
            load_monitor( size_t nSleepMillisecondsBetweenCpuLoadMeasurements = 0 ); // 0 means use g_nSleepMillisecondsBetweenCpuLoadMeasurements
		...
		namespace base64 {
			...
			inline std::string encode( const std::string & s ) { return encode( (unsigned char*)s.c_str(), s.length() ); }
	...
	public:
		...
		bool operator== ( const round_queue & other ) const { return ( compare( other ) == 0 ) ? true : false; }
		bool operator!= ( const round_queue & other ) const { return ( compare( other ) != 0 ) ? true : false; }
		bool operator<  ( const round_queue & other ) const { return ( compare( other ) <  0 ) ? true : false; }
		bool operator<= ( const round_queue & other ) const { return ( compare( other ) <= 0 ) ? true : false; }
		bool operator>  ( const round_queue & other ) const { return ( compare( other ) >  0 ) ? true : false; }
	...
	public:
		...
		bool operator== ( const with_summary & other ) const { return ( compare( other ) == 0 ) ? true : false; }
		bool operator!= ( const with_summary & other ) const { return ( compare( other ) != 0 ) ? true : false; }
		bool operator<  ( const with_summary & other ) const { return ( compare( other ) <  0 ) ? true : false; }
		bool operator<= ( const with_summary & other ) const { return ( compare( other ) <= 0 ) ? true : false; }
		bool operator>  ( const with_summary & other ) const { return ( compare( other ) >  0 ) ? true : false; }
	...
	namespace signal {
		...
		extern bool get_signal_description( int nSignalNo, std::string & strSignalName, std::string & strSignalDescription ); // returns true if signal name is known
		extern std::string signal2str( int nSignalNo, const char * strPrefix = nullptr, const char * strSuffix = nullptr, bool isAddDesc = true );

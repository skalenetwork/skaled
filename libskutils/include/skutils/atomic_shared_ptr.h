...
//			return g_mtx;
		...
		virtual bool ref_is_zombie() const {
			return ref_is_zombie_;
		}
		virtual void ref_mark_as_zombie() {
			ref_is_zombie_ = true;
		}
	...
	private:
		void throw_nullptr_access_attempt( T * ptr ) const {
			std::stringstream ss;
			ss << "retain_release_ptr/" << (typeid(ptr).name()) << "/" << ((void*)this) << " NULL access attempt";
...
//		}
		...
		T * get_unconst() const {
			return const_cast < retain_release_ptr < T, TR > * > ( this ) -> get();
		}
		void ref_retain() {
			lock_type lock( ref_mtx() );
			T * p = get();
		...
		void attach( const T * pRawOther = nullptr ) {
			attach( const_cast < T * > ( pRawOther ) );
		}
		...
		retain_release_ptr()
			: stored_( nullptr )
		{ }
		retain_release_ptr( T * pRawOther )
			: stored_( nullptr )
		{
			assign( pRawOther );
		}
		retain_release_ptr( const T * pRawOther )
			: stored_( nullptr )
		{
			assign( pRawOther );
		}
		retain_release_ptr( const retain_release_ptr < T, TR > & refOther )
			: stored_( nullptr )
		{
			assign( refOther );
		}
		retain_release_ptr( retain_release_ptr < T, TR > && refOther )
			: stored_( nullptr )
		{
			move( refOther );
		}
		...
		void reset( T * pRawOther = nullptr ) {
			assign( pRawOther );
		}
		void reset( const retain_release_ptr < T, TR > & refOther ) {
			assign( refOther );
		}
		bool empty() const {
			if( get() )
		...
		const T * operator-> () const { return get_safe(); }
			  T & operator*  ()       { return (*get_safe()); }
		const T & operator*  () const { return (*get_safe()); }
		bool operator==( const T       * pRawOther )  const { return ( compare( pRawOther ) == 0 ) ? true : false; }
		bool operator==( const retain_release_ptr < T, TR > & refOther ) const { return ( compare( refOther ) == 0 ) ? true : false; }
		bool operator!=( const T       * pRawOther )  const { return ( compare( pRawOther ) != 0 ) ? true : false; }
		bool operator!=( const retain_release_ptr < T, TR > & refOther ) const { return ( compare( refOther ) != 0 ) ? true : false; }
		bool operator< ( const T       * pRawOther )  const { return ( compare( pRawOther ) <  0 ) ? true : false; }
		bool operator< ( const retain_release_ptr < T, TR > & refOther ) const { return ( compare( refOther ) <  0 ) ? true : false; }
		...
		bool operator>=( const T       * pRawOther )  const { return ( compare( pRawOther ) >= 0 ) ? true : false; }
		bool operator>=( const retain_release_ptr < T, TR > & refOther ) const { return ( compare( refOther ) >= 0 ) ? true : false; }
		retain_release_ptr < T, TR > & operator= ( const T * pRawOther ) { assign( pRawOther ); return (*this); }
		retain_release_ptr < T, TR > & operator= ( const retain_release_ptr < T, TR > & refOther ) { assign( refOther ); return (*this); }
		retain_release_ptr < T, TR > & operator= ( retain_release_ptr < T, TR > && refOther ) { move( refOther ); return (*this); }
	...
	public:
		...
		zombie_pool( size_t max_zombie_queue_size = 256 ) : max_zombie_queue_size_( max_zombie_queue_size ), zombie_mtx_( ( "RMTX-ZOMBIE-POOL" ) ) { }

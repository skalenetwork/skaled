...
namespace skutils {
    ...
    namespace multifunction_util {
        ...
        template < typename R, typename... Args > struct multifunction_traits {
            typedef R    return_type;
        };
        template < typename R, typename Arg > struct multifunction_traits < R, Arg > {
            typedef R    return_type;
            typedef Arg  argument_type;
        };
        template < typename R, typename Arg1, typename Arg2 > struct multifunction_traits < R, Arg1, Arg2 > {
            typedef R    return_type;
            typedef Arg1 first_argument_type;
            typedef Arg2 second_argument_type;
        };

        template < typename L, typename R, typename... Args > struct invoker {
            static R invoke_all(
        ...
        template < typename L, typename... Args > struct invoker < L, void, Args... > {
    ...
    static inline multifunction_util::token_t multifunction_token_namer( void * obj, const char * desc = nullptr ) {
    ...
    template < typename R, typename... Args > class multifunction;

    template < typename R, typename... Args > class multifunction < R( Args... ) >
    ...
    public:
        ...
        template < typename F > F & get( const token_t & t ) { return map_[ t ]; }
        template < typename F > F & operator [] ( const token_t & t ) { return get( t ); }
        ...
        template < typename F > token_t push_back( const token_t & t, F fn ) {
            if( contains( t ) )
                return t;
        ...
        template < typename F > token_t push_back( F fn ) { token_t t; return push_back( t, fn ); }
        token_t push_back( const_reference item ) { return push_back( item.first, item.second ); }
        template < typename F > token_t operator += ( F fn ) { return push_back( fn ); }
        token_t operator += ( const_reference item ) { return push_back( item ); }

        template < typename F > token_t push_front( const token_t & t, F fn ) {
            if( contains( t ) )
                return t;
        ...
        template < typename F > token_t push_front( F fn ) { token_t t; return push_front( t, fn ); }
        ...
        R invoke_all( Args... args ) const {
            return
                multifunction_util::invoker < list_t, R, Args... > ::
                    invoke_all(
                        ignore_exceptions_,
                        return_on_first_false_,
                        return_on_first_true_,
                        list_,
                        args...
                        );
        }
        ...
        multifunction( bool ignore_exceptions = false, bool return_on_first_false = false, bool return_on_first_true = false )
        ...
        multifunction & operator= ( const_reference item ) { clear(); push_back( item ); return (*this); }
        multifunction & operator= ( function_t fn ) { clear(); push_back( fn ); return (*this); }
        ~multifunction() = default;

        bool ignore_exceptions() const { return ignore_exceptions_; }
        multifunction & ignore_exceptions( bool b ) { ignore_exceptions_ = b; return (*this); }
        bool return_on_first_false() const { return return_on_first_false_; }
        multifunction & return_on_first_false( bool b ) { return_on_first_false_ = b; return (*this); }
        bool return_on_first_true() const { return return_on_first_true_; }
        multifunction & return_on_first_true( bool b ) { return_on_first_true_ = b; return (*this); }

        void reset() { clear(); }
        void reset( const_reference item ) { clear(); push_back( item ); }
        void reset( const token_t & t, function_t fn ) { clear(); push_back( t, fn ); }
        void reset(                    function_t fn ) { clear(); push_back(    fn ); }

...
namespace skutils {
    namespace async {
		...
		template < typename data_src_t, typename data_dst_t, typename mutex_type = skutils::multithreading::mutex_type >
        class work {
        public:
            typedef skutils::multifunction < void( const data_src_t &, data_dst_t & ) > fn_work_handler_t;
            typedef skutils::multifunction < void( const data_src_t &, const data_dst_t &, bool ) > fn_completion_handler_t; // data, erase results flag
        private:
            typedef std::pair < std::thread::native_handle_type, fn_completion_handler_t > pair_thread_info_t;
        ...
        public:
            ...
            virtual bool request( const data_src_t & src, fn_work_handler_t fn_work_handler, fn_completion_handler_t fn_completion_handler ) {
                try {
                    lock_type lock( mutex_ );
                    if( shutdown_mode() )
                        return false;
				...
				for( ; true; ) {
                    if( needSleep ) {
						...
						if( nSleepAttemptsCount > 0 && nSleepAttemptIndex >= nSleepAttemptsCount )
							break;
						skutils::sleep_for_milliseconds( ( nSleepStepMilliseconds >= 10 ) ? nSleepStepMilliseconds : 10 );
                        needSleep = false;
                    }
					lock_type lock( mutex_ );
					if( shutdown_mode() )
		...
		template <
			typename interval_t = std::chrono::milliseconds,
			typename fn_t = std::function < void() >
			>
		class timer {
		...
		public:
			...
			timer & stop() {
				is_running_ = false;
				try { if( timer_thread_.joinable() ) timer_thread_.join(); } catch( ... ) { }
				return (*this);
			}
			...
			const interval_t & interval() const {
				return i_;
			}
			...
			timer & run( const fn_t & fn, const interval_t & i, bool is_ss = true, bool is_multi_thread = true ) {
				return stop().set_fn( fn ).set_interval( i ).set_single_shot( is_ss ).start( is_multi_thread );
			}
			timer & run( const interval_t & i, const fn_t & fn, bool is_ss = true, bool is_multi_thread = true ) {
				return stop().set_fn( fn ).set_interval( i ).set_single_shot( is_ss ).start( is_multi_thread );
			}

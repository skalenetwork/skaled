...
namespace skutils {
    ...
    extern std::list<std::string> split2list( const std::string & s, char delim, size_t nMaxSplits = std::string::npos );
    extern std::vector<std::string> split2vector( const std::string & s, char delim, size_t nMaxSplits = std::string::npos );
    ...
    public:
        ...
        class parse_error : public std::invalid_argument { // may be thrown when decoding an URL or an assigning value
        public:
        ...
        url( const std::string & url_str ) : strUrl_( url_str ), wasParsed_( false ), wasBuilt_( false ), nIPversion_( -1 ) { }
        url & operator= ( const std::string & url_str ) { return str( url_str ); }
        url & operator= ( const url & an_url) { assign( an_url ); return (*this); }
        url & operator= ( url && an_url ) { assign( std::move( an_url ) ); return (*this); }
        ...
        std::string str() const { if( ! wasBuilt_ ) compose_url(); return strUrl_; } // compose if needed and return it as string
        url & str( const std::string & url_str ) { strUrl_ = url_str; wasBuilt_ = wasParsed_ = false; return (*this); } // all fields are overwritten

        const std::string & scheme() const { lazy_parse(); return strScheme_; }
        url & scheme( const std::string & s );

        const std::string & user_info() const { lazy_parse(); return strUserName_; }
        url & user_info( const std::string & s );

        std::pair<std::string,std::string> user_name_and_password() const; // returns strings pair object, not referemnce
        url & user_name_and_password( const std::pair<std::string,std::string> & n_p, bool bRemoveIfEmpty );
        url & user_name_and_password( const std::string & strName, const std::string & strPassword, bool bRemoveIfEmpty );
        ...
        const std::string & host() const { lazy_parse(); return strHost_; }
        url & host( const std::string & h, uint8_t ip_v = 0 );

        std::int8_t ip_version() const { lazy_parse(); return nIPversion_; } // get host IP version: 0=name, 4=IPv4, 6=IPv6, -1=undefined

        const std::string & port() const { lazy_parse(); return strPort_; }
        url & port( const std::string & str );
        url & port( std::uint16_t num );

        const std::string & path() const { lazy_parse(); return strPath_; }
        ...
        const Query & query() const { lazy_parse(); return kvQuery_; }
        ...
        Query & set_query() { lazy_parse(); wasBuilt_ = false; return kvQuery_; }
        ...
        url & set_query( const Query & q )
            { lazy_parse(); if( q != kvQuery_ ) { kvQuery_ = q; wasBuilt_ = false; } return (*this); }
        url & add_query( const KeyVal & kv )
            { lazy_parse(); wasBuilt_ = false; kvQuery_.push_back( kv ); return (*this); }
        url & add_query( const std::string & key, const std::string & val )
            { lazy_parse(); wasBuilt_ = false; kvQuery_.emplace_back( key, val ); return (*this); }
        url & add_query( const std::string & key )
            { lazy_parse(); wasBuilt_ = false; kvQuery_.emplace_back( key ); return (*this); }

        const std::string & fragment() const { lazy_parse(); return strFragment_; }
        url & fragment( const std::string & f );

        std::ostream & output( std::ostream & o ) const;
        friend std::ostream & operator<< ( std::ostream & o, const url & an_url) { return an_url.output( o ); }

    private:
        void assign( const url & an_url );
        void assign( url && an_url );
        void compose_url() const;
        void lazy_parse() const { if ( ! wasParsed_ ) parse_url(); }
    ...
    public:
        ...
        bool update_user_name_and_password( const char * strOptionalUser, const char * strOptionalPassword, bool bRemoveIfEmpty ); // returns true if was updated
        bool update_user_name_and_password( const std::string & strOptionalUser, const std::string & strOptionalPassword, bool bRemoveIfEmpty ); // returns true if was updated
        bool strip_user_name_and_password(); // returns true if was updated
    }; /// class url

    extern bool update_url_user_name_and_password( url & an_url, const char * strOptionalUser, const char * strOptionalPassword, bool bRemoveIfEmpty );
    extern bool update_url_user_name_and_password( url & an_url, const std::string & strOptionalUser, const std::string & strOptionalPassword, bool bRemoveIfEmpty );
    extern bool update_url_user_name_and_password( std::string & str_url, const char * strOptionalUser, const char * strOptionalPassword, bool bRemoveIfEmpty );
    extern bool update_url_user_name_and_password( std::string & str_url, const std::string & strOptionalUser, const std::string & strOptionalPassword, bool bRemoveIfEmpty );
    extern std::string update_url_user_name_and_password_copy( const std::string & str_url, const char * strOptionalUser, const char * strOptionalPassword, bool bRemoveIfEmpty );
    extern std::string update_url_user_name_and_password_copy( const std::string & str_url, const std::string & strOptionalUser, const std::string & strOptionalPassword, bool bRemoveIfEmpty );

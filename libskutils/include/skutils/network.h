...
namespace skutils {
	namespace network {
		...
		extern e_address_family_t get_fd_name_info( int fd, int & nPort, std::string & strAddress, bool isPeer );
		extern std::string get_fd_name_as_url( int fd, const char * strUrlScheme, bool isPeer );

#if (defined SKUTILS_WITH_SSL)
        extern bool generate_self_signed_certificate_and_key_files(
		...
		extern std::string resolve_address_for_client_connection( int ipVersion, const char * ads, sockaddr46 & sa46 );
		...
		extern std::list < std::pair < std::string, std::string > > get_machine_ip_addresses( bool is4, bool is6 ); // first-interface name, second-address
		extern std::pair < std::string, std::string > get_machine_ip_address_v4(); // first-interface name, second-address
		extern std::pair < std::string, std::string > get_machine_ip_address_v6(); // first-interface name, second-address

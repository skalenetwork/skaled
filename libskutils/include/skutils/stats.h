...
namespace skutils {
	namespace stats {
        ...
        struct UnitsPerSecond
        {
            ...
            bool operator<  ( const UnitsPerSecond& unit1 ) const
            {
                return  std::tie(m_startTime, m_prevTime, m_lastTime, m_prevPerSec, m_total ) <
                        std::tie(unit1.m_startTime, unit1.m_prevTime, unit1.m_lastTime, unit1.m_prevPerSec, unit1.m_total );
            }
            bool operator> ( const UnitsPerSecond& unit1 ) const
            {
                return (*this < unit1) ? false : true;
            }
		...
		public:
            ...
            double compute_eps(const std::string& name, const time_point& tpNow, size_t * p_nSummary = nullptr) const;
            double compute_eps(t_NamedEventsIt& currentIt, const time_point& tpNow, size_t * p_nSummary = nullptr) const;
			virtual std::string getEventStatsDescription( time_point tpNow, bool isColored = false, bool bSkipEmptyStats = true, bool bWithSummaryAsSuffix = false ) const;
			std::string getEventStatsDescription( bool isColored = false, bool bSkipEmptyStats = true, bool bWithSummaryAsSuffix = false ) const;
            virtual nlohmann::json toJSON( time_point tpNow, bool bSkipEmptyStats = true, const std::string& prefix = "items" ) const;
		...
		typedef skutils::with_summary < bytes_count_t, skutils::round_queue < traffic_record_item_t > > traffic_queue_t;

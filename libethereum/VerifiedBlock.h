...
#ifdef BITE
    std::shared_ptr< std::map< uint64_t, std::shared_ptr< bytes > > > decryptedTransactionDataFields = std::make_shared<std::map<uint64_t, std::shared_ptr<bytes>>>();  ///< Decrypted
                                                                                                                                                                        ///< transaction
                                                                                                                                                                        ///< data fields
                                                                                                                                                                        ///< to be stored
                                                                                                                                                                        ///< in blockchain
...
struct VerifiedBlock {
    ...
    VerifiedBlock( BlockHeader&& _bi ) { verified.info = std::move( _bi ); }

    VerifiedBlock( VerifiedBlock&& _other )
        : verified( std::move( _other.verified ) ), blockData( std::move( _other.blockData ) ) {}

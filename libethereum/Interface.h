...
enum class Reaping { Automatic, Manual };

enum class FudgeFactor { Strict, Lenient };
...
public:
    ...
    void submitTransaction( Secret const& _secret, u256 const& _value, Address const& _dest,
        bytes const& _data = bytes(), u256 const& _gas = 1000000,
    ...
    /// Get a map containing each of the pending transactions (transactions from accounts managed by
    /// this node which have not yet made it into a mined block)
    ...
    virtual TransactionSkeleton populateTransactionWithDefaults(
        TransactionSkeleton const& _t ) const = 0;

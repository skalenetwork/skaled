...
// TODO This is really used only in tests. So it can safely crash %)
void BlockQueue::verifierBody() try {
    while ( !m_deleting ) {
        UnverifiedBlock work;
        {
            MICROPROFILE_SCOPEI( "BlockQueue", "unique_lock<Mutex> l(m_verification)", MP_DIMGRAY );
            unique_lock< Mutex > l( m_verification );
            {
                ...
                // );
                for ( ; true; ) {
                    if ( m_moreToVerify.wait_for( l, std::chrono::milliseconds( 500 ),
        ...
        try {
            res.verified =
                m_bc->verifyBlock( &res.blockData, m_onBad, ImportRequirements::OutOfOrderChecks );
...
ImportResult BlockQueue::import( bytesConstRef _block, bool _isOurs ) {
    MICROPROFILE_SCOPEI( "BlockQueue", "import", MP_PERU );
    ...
    try {
        ...
        BOOST_LOG( m_loggerWarning ) << "Ignoring malformed block: " << diagnostic_information( _e );
    ...
    if ( bi.timestamp() > utcTime() && !_isOurs ) {
        ...
        BOOST_LOG( m_loggerTrace ) << "OK - queued for future [" << bi.timestamp() << " vs " << utcTime()
                             << "] - will wait until " << buf;
        ...
        } else if ( !m_readySet.count( bi.parentHash() ) &&
...
void BlockQueue::updateBad_WITH_LOCK( h256 const& _bad ) {
    ...
    DEV_GUARDED( m_verification ) {
        ...
        while ( moreBad ) {
            moreBad = false;
            std::vector< VerifiedBlock > badVerified =
                m_verified.removeIf( [this]( VerifiedBlock const& _b ) {
                    return m_knownBad.count( _b.verified.info.parentHash() ) ||
                           m_knownBad.count( _b.verified.info.hash() );
                } );
            ...
            std::vector< UnverifiedBlock > badUnverified =
                m_unverified.removeIf( [this]( UnverifiedBlock const& _b ) {
            ...
            std::vector< VerifiedBlock > badVerifying =
                m_verifying.removeIf( [this]( VerifiedBlock const& _b ) {
                    return m_knownBad.count( _b.verified.info.parentHash() ) ||
                           m_knownBad.count( _b.verified.info.sha3Uncles() );
                } );
            for ( auto& b : badVerifying ) {
                h256 const& h =
                    b.blockData.size() != 0 ? b.verified.info.hash() : b.verified.info.sha3Uncles();
                m_knownBad.insert( h );
                m_readySet.erase( h );
...
void BlockQueue::collectUnknownBad_WITH_BOTH_LOCKS( h256 const& _bad ) {
    ...
    while ( !badQueue.empty() ) {
        vector< pair< h256, bytes > > const removed =
...
bool BlockQueue::invariants() const {
    ...
    if ( m_readySet.size() != knownCount() ) {
        std::stringstream s;
        s << "Failed BlockQueue invariant: m_readySet: " << m_readySet.size()
          << " m_verified: " << m_verified.count() << " m_unverified: " << m_unverified.count()

...
#ifdef BITE
    const auto epochId = m_client.getCurrentEpochId();
    BOOST_LOG( m_loggerInfo ) << "BITE epoch passed to consensus at startup: epochId=" << epochId
                              << " blockNumber=" << m_client.number();
    consensusEnginePtr->setEpochId( epochId );
#endif // BITE

...
public:
    ...
    bool start( std::string const& _sealer ) {
        ...
        for ( unsigned i = 0; i < ins; ++i ) {
            m_miners.push_back( std::shared_ptr< Miner >(
                m_sealers[_sealer].create( std::make_pair( this, i ) ) ) );
    ...
    WorkingProgress const& miningProgress() const {
        WorkingProgress p;
        p.ms = std::chrono::duration_cast< std::chrono::milliseconds >(
            std::chrono::steady_clock::now() - m_lastStart )
                   .count();
        {
...
private:
    void resetTimer() { m_lastStart = std::chrono::steady_clock::now(); }

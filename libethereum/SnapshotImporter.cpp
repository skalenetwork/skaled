...
void SnapshotImporter::import(
    ...
    LOG( m_logger ) << "Importing snapshot for block " << blockNumber << " block hash "
                    << blockHash;
...
void SnapshotImporter::importStateChunks( SnapshotStorageFace const& _snapshotStorage,
    ...
    for ( auto const& stateChunkHash : _stateChunkHashes ) {
        ...
        for ( size_t accountIndex = 0; accountIndex < accountCount; ++accountIndex ) {
            ...
            _byte_ const codeFlag = account[2].toInt< _byte_ >( RLP::VeryStrict );
            h256 codeHash;
            switch ( codeFlag ) {
            case 0:
                codeHash = EmptySHA3;
            ...
            case 1:
                codeHash =
                    m_stateImporter.importCode( account[3].toBytesConstRef( RLP::VeryStrict ) );
                break;
            case 2:

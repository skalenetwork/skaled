...
inline std::ostream& operator<<( std::ostream& _out, WorkingProgress _p ) {
    _out << _p.rate() << " H/s = " << _p.hashes << " hashes / " << ( double( _p.ms ) / 1000 )
         << " s";
    return _out;
}
...
public:
     ...
     * @param _wp The WorkPackage that the Solution is for; this will be reset if the work is
     * accepted.
...
public:
    ...
    GenericMiner( ConstructionInfo const& _ci ) : m_farm( _ci.first ), m_index( _ci.second ) {}

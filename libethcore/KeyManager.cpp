...
bool KeyManager::exists() const {
    return !contents( appendToFilename( m_keysFile, ".salt" ) ).empty() &&
           !contents( m_keysFile ).empty();
}
...
bool KeyManager::recode( Address const& _address, string const& _newPass, string const& _hint,
            ...
            for ( auto const& i : s[1] ) {
                ...
                if ( uuid ) {
                    if ( m_store.contains( uuid ) ) {
                        m_addrLookup[addr] = uuid;
                        m_uuidLookup[uuid] = addr;
                        m_keyInfo[addr] = KeyInfo(
                            h256( i[2] ), string( i[3] ), i.itemCount() > 4 ? string( i[4] ) : "" );
                    ...
                    m_keyInfo[addr] = KeyInfo(
                        h256( i[2] ), string( i[3] ), i.itemCount() > 4 ? string( i[4] ) : "" );
                //				cdebug << toString(addr) << toString(uuid) << toString((h256)i[2])
                //<< (string)i[3];
            }
            if ( saveRequired )
                m_store.save();
...
Secret KeyManager::secret(
    Address const& _address, function< string() > const& _pass, bool _usePasswordCache ) const {
    return secret( m_addrLookup.at( _address ), _pass, _usePasswordCache );
}

Secret KeyManager::secret(
    h128 const& _uuid, function< string() > const& _pass, bool _usePasswordCache ) const {
    if ( _usePasswordCache )
        return Secret( m_store.secret(
            _uuid, [&]() { return getPassword( _uuid, _pass ); }, _usePasswordCache ) );
    else
...
h128 KeyManager::import( Secret const& _s, string const& _accountName, string const& _pass,
    string const& _passwordHint ) {
...
void KeyManager::importExisting(
    h128 const& _uuid, string const& _info, string const& _pass, string const& _passwordHint ) {
...
void KeyManager::importExisting( h128 const& _uuid, string const& _accountName,
    Address const& _address, h256 const& _passHash, string const& _passwordHint ) {
...
KeyPair KeyManager::presaleSecret(
    std::string const& _json, function< string( bool ) > const& _password ) {
    js::mValue val;
...
h256 KeyManager::hashPassword( string const& _pass ) const {
    ...
    return h256(
        pbkdf2( _pass, asBytes( m_defaultPasswordDeprecated ), 262144, 32 ).makeInsecure() );
}
...
KeyPair KeyManager::newKeyPair( KeyManager::NewKeyType _type ) {
    ...
    auto f = [&]() {
        ...
        while ( keepGoing ) {
            ...
            lp = KeyPair::create();
            auto a = lp.address();
            if ( _type == NewKeyType::NoVanity || ( _type == NewKeyType::DirectICAP && !a[0] ) ||
                 ...
                 ( _type == NewKeyType::FirstFour && a[0] == a[1] && a[1] == a[2] &&
                     a[2] == a[3] ) )
                break;
        }
        if ( keepGoing )

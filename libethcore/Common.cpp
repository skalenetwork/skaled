...
#if ETH_FATDB
const unsigned c_minorProtocolVersion = 4;
...
const bytes c_blockhashContractCode( fromHex(
    "0x600073fffffffffffffffffffffffffffffffffffffffe33141561005957600143035b6001156100535760003561"
    "0100820683015561010081061561004057005b6101008104905061010082019150610022565b506100e0565b436000"
...
const Address c_accessControllerContractAddress( "D2002000000000000000000000000000000000D2" );
...
static void badBlockInfo( BlockHeader const& _bi, string const& _err ) {
    ...
    ss << c_border + "  Import Failure     " + _err +
              string( max< int >( 0, 53 - _err.size() ), ' ' ) + "  " + c_border
       << "\n";
    ss << c_space << "\n";
    string bin = toString( _bi.number() );
    ss << c_border +
              ( "                     Bad Block #" +
                  string( max< int >( 0, 8 - bin.size() ), '0' ) + bin + "." +
                  _bi.hash().abridged() + "                    " ) +
              c_border
       << "\n";
    ss << c_space << "\n";
...
string TransactionSkeleton::userReadable( bool _toProxy,
    function< pair< bool, string >( TransactionSkeleton const& ) > const& _getNatSpec,
    function< string( Address const& ) > const& _formatAddress ) const {
    if ( creation ) {
        // show notice concerning the creation code. TODO: this needs entering into natspec.
        return string( "ÐApp is attempting to create a contract; " ) +
    ...
    if ( !isContract ) {
        // recipient has no code - nothing special about this transaction, show basic value transfer
        // info
        return "ÐApp is attempting to send " + formatBalance( value ) + " to a recipient " +
               _formatAddress( to ) +
               ( _toProxy ? " (this transaction is not executed directly, but forwarded to another "
                            "ÐApp)" :
                            "" ) +
    ...
    if ( natSpec.empty() )
        return "ÐApp is attempting to call into an unknown contract at address " +
               ...
               ( _toProxy ? "This transaction is not executed directly, but forwarded to another "
                            "ÐApp.\n\n" :
                            "" ) +
               "Call involves sending " + formatBalance( value ) +
               " to the recipient, with additional network fees of up to " +
               formatBalance( gas * gasPrice ) +
               "However, this also does other stuff which we don't understand, and does so in your "
               "name.\n\n" +
               "WARNING: This is probably going to cost you at least " +
               formatBalance( value + gas * gasPrice ) +
               ", however this doesn't include any side-effects, which could be of far greater "
               "importance.\n\n" +
               "REJECT UNLESS YOU REALLY KNOW WHAT YOU ARE DOING!";

    return "ÐApp attempting to conduct contract interaction with " + _formatAddress( to ) +
           ": <b>" + natSpec + "</b>.\n\n" +
           ( _toProxy ?
                   "This transaction is not executed directly, but forwarded to another ÐApp.\n\n" :
                   "" ) +
           ( value > 0 ?
                   "In addition, ÐApp is attempting to send " + formatBalance( value ) +

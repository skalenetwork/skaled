...
int main( int argc, char** argv ) {
    ...
    addTransactionOption( "gas-price", po::value< u256 >(),
        "<n> Transaction's gas price' should be <n> (default: 0)." );
    ...
    addGeneralOption( "number",
        po::value< int64_t >()
            ->default_value( 0 )
            ->value_name( "<number>" )
            ->notifier( [&]( int64_t _n ) { blockHeader.setNumber( _n ); } ),
        "<n> Set number" );
    ...
    if ( !inputFile.empty() ) {
        ...
        if ( inputFile == "-" )
            for ( int i = cin.get(); i != -1; i = cin.get() )
                code.push_back( static_cast< _byte_ >( i ) );
        else
    ...
    unordered_map< _byte_, pair< unsigned, bigint > > counts;
    unsigned total = 0;
    bigint memTotal;
    ...
    if ( mode == Mode::Statistics ) {
        onOp = [&]( uint64_t, uint64_t, Instruction inst, bigint m, bigint gasCost, bigint,
            ...
            _byte_ b = static_cast< _byte_ >( inst );
            counts[b].first++;

...
std::string hexPrefixEncode(
    ...
    unsigned end = ( _endNibble < 0 ? ( ( int ) ( _data.size() * 2 - _offset ) + 1 ) + _endNibble :
                                      _endNibble ) +
                   _offset;
...
_byte_ uniqueInUse( RLP const& _orig, _byte_ except ) {
    _byte_ used = 255;

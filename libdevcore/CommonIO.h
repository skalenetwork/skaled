...
inline void writeFile(
    boost::filesystem::path const& _file, bytes const& _data, bool _writeDeleteRename = false ) {
...
void copyDirectory(
    boost::filesystem::path const& _srcDir, boost::filesystem::path const& _dstDir );
...
#endif
{
    ...
    std::chrono::time_point< std::chrono::system_clock > const tpAfterDuration(
        durationSinceEpoch );
...
template < class T, std::size_t Z >
inline std::ostream& streamout( std::ostream& _out, std::array< T, Z > const& _e )  // used
                                                                                    // somewhere?
{
    _out << "[";
    if ( !_e.empty() ) {

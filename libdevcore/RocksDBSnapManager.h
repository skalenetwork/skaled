...
class RocksDBSnapManager {

public:
    const std::shared_ptr< RocksDBSnap >& getLastBlockSnap() const;
    ...
    void closeAllOpenSnaps(std::unique_ptr< rocksdb::DB >& _db, uint64_t  _dbInstanceId);
...
private:
    ...
    std::map<std::uint64_t , std::shared_ptr<RocksDBSnap>> oldSnaps;
    std::shared_ptr<RocksDBSnap> m_lastBlockSnap;
    ...
    void createNewSnap( uint64_t _blockId, std::unique_ptr< rocksdb::DB >& _db, uint64_t _dbInstanceId );
};

...
using errinfo_rocksdbStatusCode =
    boost::error_info< struct tag_rocksdbStatusCode, rocksdb::Status::Code >;
using errinfo_rocksdbStatusSubCode =
    boost::error_info< struct tag_rocksdbStatusSubCode, rocksdb::Status::SubCode >;
...
void RocksDBWriteBatch::insert( Slice _key, Slice _value ) {
    auto const status = m_writeBatch.Put( rocksdb::Slice( _key.data(), _key.size() ),
...
bool RocksDB::exists( Slice _key ) const {
    std::string value;
    rocksdb::Slice const key( _key.data(), _key.size() );
    if ( !m_db->KeyMayExist( m_readOptions, key, &value, ( std::string* ) nullptr ) )
        return false;
...
void RocksDB::commit( std::unique_ptr< WriteBatchFace > _batch ) {
    ...
    if ( !batchPtr )
        BOOST_THROW_EXCEPTION(
            DatabaseError() << errinfo_comment( "Invalid batch type passed to rocksdb::commit" ) );

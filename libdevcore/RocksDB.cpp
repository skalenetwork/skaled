...
std::string RocksDB::lookup( Slice _key, const std::shared_ptr< RocksDBSnap >& _snap ) const {
...
bool RocksDB::exists( Slice _key, const std::shared_ptr< RocksDBSnap >& _snap ) const {
    std::string value;
    rocksdb::Slice const key( _key.data(), _key.size() );
...
rocksdb::Status RocksDB::getValue( rocksdb::ReadOptions _readOptions, const rocksdb::Slice& _key,
    ...
    if ( _snap ) {
        ...
        // db handle
        RDB_CHECK( m_dbReopenId == _snap->getParentDbReopenId() );
...
void RocksDB::forEachWithPrefix( std::string& _prefix, std::function< bool( Slice, Slice ) > _f)
    const {
...
void RocksDB::createBlockSnap( uint64_t _blockNumber ) {
...
std::shared_ptr< RocksDBSnap > RocksDB::getLastBlockSnap() const {

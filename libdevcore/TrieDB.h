...
public:
    ...
    /// True if the trie is initialised but empty (i.e. that the DB contains the root node which is
    /// empty).
    ...
    private:
        ...
        struct Node {
            ...
            _byte_ child;     // 255 -> entering, 16 -> actually at the node, 17 -> exiting, 0-15 ->
                              // actual children.
    ...
    void descendKey( h256 const& _k, h256Hash& _keyMask, bool _wasExt, std::ostream* _out,
        int _indent = 0 ) const {
        _keyMask.erase( _k );
    ...
    void descendList(
        ...
        if ( _r.isList() && _r.itemCount() == 2 && ( !_wasExt || _out ) ) {
            ...
            if ( _out )
                ( *_out ) << std::string( _indent * 2, ' ' ) << "17 " << sha3( _r.data() ) << ": "
                          << _r << "\n";
...
private:
    ...
    // out2: [k{}, S]      where i == 16
    bytes merge( RLP const& _orig, _byte_ _i );
    ...
    // -- OR --
    // in: [k{i}K, V] (DEL)
    // out: [null ** i, H, null ** (16 - i)] ; [K, V] => H (INS)  (being [null ** i, [K, V], null **
    // (16 - i)]  if necessary)
...
std::ostream& operator<<( std::ostream& _out, GenericTrieDB< DB > const& _db ) {
    for ( auto const& i : _db )
        _out << escaped( i.first.toString(), false ) << ": "
             << escaped( i.second.toString(), false ) << std::endl;
    return _out;
}
...
public:
    ...
    SpecificTrieDB( DB* _db, h256 _root, Verification _v = Verification::Normal )
        : Generic( _db, _root, _v ) {}

    std::string operator[]( KeyType _k ) const { return at( _k ); }

    bool contains( KeyType _k ) const {
        return Generic::contains( bytesConstRef( ( _byte_ const* ) &_k, sizeof( KeyType ) ) );
    }
    std::string at( KeyType _k ) const {
        return Generic::at( bytesConstRef( ( _byte_ const* ) &_k, sizeof( KeyType ) ) );
    }
    void insert( KeyType _k, bytesConstRef _value ) {
        Generic::insert( bytesConstRef( ( _byte_ const* ) &_k, sizeof( KeyType ) ), _value );
    }
    void insert( KeyType _k, bytes const& _value ) { insert( _k, bytesConstRef( &_value ) ); }
    void remove( KeyType _k ) {
        Generic::remove( bytesConstRef( ( _byte_ const* ) &_k, sizeof( KeyType ) ) );
    }
    ...
    iterator lower_bound( KeyType _k ) const {
        return iterator( this, bytesConstRef( ( _byte_ const* ) &_k, sizeof( KeyType ) ) );
    }
};
...
public:
    ...
    using Super::setRootBlockNumber;
#endif
...
public:
    ...
    public:
        ...
        HashedIterator( FatGenericTrieDB const* _trie, bytesConstRef _hashedKey )
            : Super( _trie, _hashedKey ) {}

        bytes key() const {
            auto hashed = Super::at();
            return static_cast< FatGenericTrieDB const* >( Super::m_that )
                ->db()
                ->lookupAux( h256( hashed.first ) );
        }
    };
    ...
    HashedIterator hashedLowerBound( h256 const& _hashedKey ) const {
        return HashedIterator( this, _hashedKey.ref() );
    }
};
...
typename GenericTrieDB< DB >::iterator::value_type GenericTrieDB< DB >::iterator::at() const {
    ...
    assert( b.key.size() );
    assert( !( b.key[0] & 0x10 ) );  // should be an integer number of bytes (i.e. not an odd number
                                     // of nibbles).

    RLP rlp( b.rlp );
    return std::make_pair(
        bytesConstRef( b.key ).cropped( 1 ), rlp[rlp.itemCount() == 2 ? 1 : 16].payload() );
}
    ...
    while ( true ) {
        ...
        if ( m_trail.back().child == 255 ) {
            ...
            if ( rlp.itemCount() == 2 ) {
                ...
                // TODO: do something different depending on how keyOfRLP compares to k.mid(0,
                // std::min(k.size(), keyOfRLP.size())); if == all is good - continue descent. if >
                // discard key and continue descent. if < discard key and skip node.
                ...
                m_trail.back().key =
                    hexPrefixEncode( keyOf( m_trail.back().key ), keyOfRLP, false );
                if ( isLeaf( rlp ) ) {
                    // leaf - exit now.
                    if ( k.empty() ) {
                    ...
                    // Still data in key we're supposed to be looking for when we're at a leaf. Go
                    // for next one.
                    k.clear();
                    m_trail.pop_back();
                    continue;
    ...
    while ( true ) {
        ...
        if ( m_trail.back().child == 255 ) {
            ...
            if ( rlp.itemCount() == 2 ) {
                // Just turn it into a valid Branch
                m_trail.back().key =
                    hexPrefixEncode( keyOf( m_trail.back().key ), keyOf( rlp ), false );
                if ( isLeaf( rlp ) ) {
                    // leaf - exit now.
                    m_trail.back().child = 0;
                    return;
...
template < class KeyType, class DB >
typename SpecificTrieDB< KeyType, DB >::iterator::value_type
SpecificTrieDB< KeyType, DB >::iterator::at() const {
    auto p = Super::at();
    value_type ret;
...
void GenericTrieDB< DB >::insert( bytesConstRef _key, bytesConstRef _value ) {
    ...
    // So, if it's less than 32 (and thus should have been deleted but wasn't) then we delete it
    // here.
...
template < class DB >
bytes GenericTrieDB< DB >::mergeAt(
    RLP const& _orig, NibbleSlice _k, bytesConstRef _v, bool _inLine ) {
    return mergeAt( _orig, sha3( _orig.data() ), _k, _v, _inLine );
}

template < class DB >
bytes GenericTrieDB< DB >::mergeAt(
    RLP const& _orig, h256 const& _origHash, NibbleSlice _k, bytesConstRef _v, bool _inLine ) {
...
template < class DB >
void GenericTrieDB< DB >::mergeAtAux(
    RLPStream& _out, RLP const& _orig, NibbleSlice _k, bytesConstRef _v ) {
    RLP r = _orig;
    std::string s;
    // _orig is always a segment of a node's RLP - removing it alone is pointless. However, if may
    // be a hash, in which case we deref and we know it is removable.
...
bytes GenericTrieDB< DB >::deleteAt( RLP const& _orig, NibbleSlice _k ) {
    ...
    if ( _orig.itemCount() == 2 ) {
        ...
        if ( _k.size() == 0 && !_orig[16].isEmpty() ) {
            ...
            _byte_ used = uniqueInUse( _orig, 16 );
            if ( used != 255 )
            ...
            for ( _byte_ i = 0; i < 17; ++i )
                if ( i == n )
                    if ( !deleteAtAux( r, _orig[i], _k.mid( 1 ) ) )  // bomb out if the key didn't
                                                                     // turn up.
                        return bytes();
                    else {
                    }
            ...
            RLP rlp( r.out() );
            _byte_ used = uniqueInUse( rlp, 255 );
...
bool GenericTrieDB< DB >::deleteAtAux( RLPStream& _out, RLP const& _orig, NibbleSlice _k ) {
    bytes b = _orig.isEmpty() ?
                  bytes() :
                  deleteAt( _orig.isList() ? _orig : RLP( node( _orig.toHash< h256 >() ) ), _k );
...
template < class DB >
bytes GenericTrieDB< DB >::merge( RLP const& _orig, _byte_ _i ) {
        ...
        for ( unsigned i = 0; i < 16; ++i )
            if ( i == b )
                if ( isLeaf( _orig ) || k.size() > 1 )
                    streamNode(
                        r, rlpList( hexPrefixEncode( k.mid( 1 ), isLeaf( _orig ) ), _orig[1] ) );
                else
                    r << _orig[1];
            else
                r << "";

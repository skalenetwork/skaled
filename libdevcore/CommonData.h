...
std::string toHex( Iterator _it, Iterator _end, std::string const& _prefix ) {
    ...
    static_assert(
        sizeof( typename traits::value_type ) == 1, "toHex needs byte-sized element type" );
...
/// Converts a string to a byte array containing the string's (_byte_) data.
inline bytes asBytes( std::string const& _b ) {
    return bytes( ( _byte_ const* ) _b.data(), ( _byte_ const* ) ( _b.data() + _b.size() ) );
}

...
public:
    ...
    explicit FixedHash( _byte_ const* _bs, ConstructFromPointerType ) {
    ...
    explicit operator bool() const {
        return std::any_of( m_data.begin(), m_data.end(), []( _byte_ _b ) { return _b != 0; } );
    }
    ...
    _byte_& operator[]( unsigned _i ) { return m_data[_i]; }
    /// @returns a particular byte from the hash.
    _byte_ operator[]( unsigned _i ) const { return m_data[_i]; }
    ...
    _byte_* data() { return m_data.data(); }

    /// @returns a constant byte pointer to the object's data.
    _byte_ const* data() const { return m_data.data(); }

    /// @returns begin iterator.
    auto begin() const -> typename std::array< _byte_, N >::const_iterator {
        return m_data.begin();
    }

    /// @returns end iterator.
    auto end() const -> typename std::array< _byte_, N >::const_iterator { return m_data.end(); }

    /// @returns a copy of the object's data as a byte vector.
    bytes asBytes() const { return bytes( data(), data() + N ); }

    /// @returns a mutable reference to the object's data as an STL array.
    std::array< _byte_, N >& asArray() { return m_data; }

    /// @returns a constant reference to the object's data as an STL array.
    std::array< _byte_, N > const& asArray() const { return m_data; }
...
private:
    std::array< _byte_, N > m_data;  ///< The binary data.
};
...
public:
    ...
    explicit SecureFixedHash(
        ...
        : FixedHash< T >( _b, _t ) {}
    explicit SecureFixedHash(
        bytesSec const& _b, ConstructFromHashType _t = FixedHash< T >::FailIfDifferent )
    ...
    bool operator==( SecureFixedHash const& _c ) const {
        return static_cast< FixedHash< T > const& >( *this ).operator==(
            static_cast< FixedHash< T > const& >( _c ) );
    }
    bool operator!=( SecureFixedHash const& _c ) const {
        return static_cast< FixedHash< T > const& >( *this ).operator!=(
            static_cast< FixedHash< T > const& >( _c ) );
...
#pragma GCC diagnostic ignored "-Wdeprecated-copy"
    ...
    SecureFixedHash& operator^=( SecureFixedHash const& _c ) {
        static_cast< FixedHash< T >& >( *this ).operator^=(
            static_cast< FixedHash< T > const& >( _c ) );
        return *this;
    }

...
public:
    ...
    RangeMask lowest( unsigned _items ) const {
        ...
        for ( auto i = m_ranges.begin(); i != m_ranges.end() && _items; ++i )
            _items -=
                ( ret.m_ranges[i->first] = std::min( i->first + _items, i->second ) ) - i->first;
        return ret;
    }
    ...
    RangeMask& operator-=( S const& _m ) {
        return invert().unionWith( _m ).invert();
    }
    ...
    /// Adds the single element _i to the range mask.
    RangeMask& unionWith( unsigned _i ) { return operator+=( Range( _i, _i + 1 ) ); }
    ...
    bool full() const {
        return m_all.first == m_all.second ||
               ( m_ranges.size() == 1 && m_ranges.begin()->first == m_all.first &&
                   m_ranges.begin()->second == m_all.second );
    }
    ...
    void extendAll( unsigned _i ) {
        m_all = std::make_pair( std::min( m_all.first, _i ), std::max( m_all.second, _i + 1 ) );
    }
    ...
    public:
        ...
        bool operator==( const_iterator const& _i ) const {
            return m_owner == _i.m_owner && m_value == _i.m_value;
        }
    ...
    private:
        const_iterator( RangeMask const& _m, bool _end )
            : m_owner( &_m ),
              m_value(
                  _m.m_ranges.empty() || _end ? _m.m_all.second : _m.m_ranges.begin()->first ) {}
...
RangeMask& RangeMask::unionWith( RangeMask::Range const& _m ) {
    for ( auto i = _m.first; i < _m.second; ) {
        ...
        // For each number, we find the element equal or next lower. this, if any, must contain the
        // value. First range that starts after i.

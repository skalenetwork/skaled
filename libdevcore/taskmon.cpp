...
vector< string > taskmon::lists_diff( const vector< string >& from, const vector< string >& to ) {
    ...
    // keep removed
    new_end = remove_if( from_copy.begin(), from_copy.end(),
        [&to]( const string& s ) -> bool { return find( to.begin(), to.end(), s ) != to.end(); } );
    from_copy.erase( new_end, from_copy.end() );

    // keep added
    new_end = remove_if( to_copy.begin(), to_copy.end(), [&from]( const string& s ) -> bool {
        return find( from.begin(), from.end(), s ) != from.end();
    } );
    to_copy.erase( new_end, to_copy.end() );

    // combine
    for_each( from_copy.begin(), from_copy.end(),
        [&diff]( const string& s ) { diff.push_back( "-" + s ); } );
    for_each(
        to_copy.begin(), to_copy.end(), [&diff]( const string& s ) { diff.push_back( "+" + s ); } );

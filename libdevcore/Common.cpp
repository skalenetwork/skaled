...
void ExitHandler::exitHandler( int nSignalNo, ExitHandler::exit_code_t ec ) {
    ...
    cerr << strMessagePrefix << skutils::signal::signal2str( nSignalNo ) << "\n\n";
...
void InvariantChecker::checkInvariants(
    ...
    if ( !_this->invariants() ) {
        cwarn << ( _pre ? "Pre" : "Post" ) << "invariant failed in" << _fn << "at" << _file << ":"
              << _line;
        boost::throw_exception( FailedInvariant() << boost::throw_function(_fn) << boost::throw_file(_file) << boost::throw_line(_line) );
    }
}

...
RLP::RLP( bytesConstRef _d, Strictness _s ) : m_data( _d ) {
    if ( ( _s & FailIfTooBig ) && actualSize() < _d.size() ) {
        if ( _s & ThrowOnFail )
            BOOST_THROW_EXCEPTION(
                OversizeRLP() << constructRLPSizeErrorInfo( actualSize(), _d.size() ) );
        else
            m_data.reset();
    }
    if ( ( _s & FailIfTooSmall ) && actualSize() > _d.size() ) {
        if ( _s & ThrowOnFail )
            BOOST_THROW_EXCEPTION(
                UndersizeRLP() << constructRLPSizeErrorInfo( actualSize(), _d.size() ) );
        else
            m_data.reset();
    }
}
    ...
    while ( m_listStack.size() ) {
        if ( m_listStack.back().first < _itemCount )
            BOOST_THROW_EXCEPTION( RLPException()
                                   ...
                                   << RequirementError( ( bigint ) m_listStack.back().first,
                                          ( bigint ) _itemCount ) );
...
void RLPStream::pushCount( size_t _count, _byte_ _base ) {
